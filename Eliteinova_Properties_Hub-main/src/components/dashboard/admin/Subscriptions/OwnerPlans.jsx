// src/components/admin/Subscriptions/OwnerPlans.jsx
...
import { FaCity, FaCrown, FaGem, FaHome, FaHotel, FaRupeeSign } from 'react-icons/fa';
...
// ============================================================
// DEFAULT PLAN TYPES - Clean version (no limits/capabilities)
// ============================================================
...
const PROPERTY_TYPE_CONFIG = {
  'Individual': { icon: FiUser, color: 'from-sky-600 to-sky-400', bg: 'bg-sky-50', text: 'text-sky-700', border: 'border-sky-200' },
  'Apartment': { icon: FaHome, color: 'from-teal-600 to-teal-400', bg: 'bg-teal-50', text: 'text-teal-700', border: 'border-teal-200' },
  'Commercial': { icon: FiBriefcase, color: 'from-indigo-600 to-indigo-400', bg: 'bg-indigo-50', text: 'text-indigo-700', border: 'border-indigo-200' },
  'Land & Plots': { icon: FiMap, color: 'from-amber-700 to-amber-500', bg: 'bg-amber-50', text: 'text-amber-800', border: 'border-amber-200' },
  'Hostel': { icon: FaHotel, color: 'from-rose-600 to-rose-400', bg: 'bg-rose-50', text: 'text-rose-700', border: 'border-rose-200' }
};
...
// ============================================================
const ViewOwnerDetailModal = ({ owner, show, onClose, onEdit, onDelete, planTypes }) => {
  if (!owner || !show) return null;

  const planConfig = planTypes[owner.subscriptionPlan] || planTypes['Free'] || { theme: FALLBACK_THEME, label: owner.subscriptionPlan, icon: FiPackage };
  const theme = planConfig.theme || FALLBACK_THEME;
  const PlanIcon = planConfig.icon || FiPackage;
  const propTypeConfig = PROPERTY_TYPE_CONFIG[owner.propertyType] || PROPERTY_TYPE_CONFIG['Individual'];
      ...
      <div className="bg-white rounded-3xl max-w-4xl w-full max-h-[90vh] overflow-hidden shadow-2xl animate-slide-up border border-[#E8F0EE] flex flex-col">
        <div className="sticky top-0 p-6 rounded-t-3xl z-10 shrink-0" style={{ background: theme.gradient }}>
          ...
          <div className="flex items-center gap-2 flex-wrap">
            <span className={`px-3 py-1 rounded-full text-xs font-semibold flex items-center gap-1.5 ${propTypeConfig.bg} ${propTypeConfig.text} border ${propTypeConfig.border}`}>
              <PropTypeIcon className="text-xs" /> {owner.propertyType}
            </span>
          </div>
        </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="bg-[#F5F9F8] rounded-2xl p-4">
              <div className="flex items-center gap-2 mb-1">
                ...
                <h4 className="text-xs font-semibold text-[#5A7D78] uppercase tracking-wider">Owner ID</h4>
              </div>
              <p className="text-sm font-bold text-[#1A2E2A]">{owner.ownerId}</p>
            </div>
            ...
            <div className="bg-[#F5F9F8] rounded-2xl p-4">
              ...
              <p className="text-sm font-bold text-[#1A2E2A]">{owner.name}</p>
            </div>
            ...
            <div className="bg-[#F5F9F8] rounded-2xl p-4">
              ...
              <p className="text-sm font-bold text-[#1A2E2A] truncate">{owner.email}</p>
            </div>
            ...
            <div className="bg-[#F5F9F8] rounded-2xl p-4">
              ...
              <p className="text-sm font-bold text-[#1A2E2A]">{owner.phone}</p>
            </div>
            ...
            <div className="bg-[#F5F9F8] rounded-2xl p-4">
              ...
              <p className="text-sm font-bold text-[#1A2E2A]">{owner.street}</p>
            </div>
            ...
            <div className="bg-[#F5F9F8] rounded-2xl p-4">
              ...
              <p className="text-sm font-bold text-[#1A2E2A]">{owner.area}</p>
            </div>
            ...
            <div className="bg-[#F5F9F8] rounded-2xl p-4">
              ...
              <p className="text-sm font-bold text-[#1A2E2A]">{owner.city}</p>
            </div>
            ...
            <div className="bg-[#F5F9F8] rounded-2xl p-4">
              ...
              <p className="text-sm font-bold text-[#1A2E2A]">{owner.district}</p>
            </div>
            ...
            <div className="bg-[#F5F9F8] rounded-2xl p-4">
              ...
              <p className="text-sm font-bold text-[#1A2E2A]">{owner.state}</p>
            </div>
            ...
            <div className="bg-[#F5F9F8] rounded-2xl p-4">
              ...
              <p className="text-sm font-bold text-[#1A2E2A]">{owner.country}</p>
            </div>
            ...
            <div className="bg-[#F5F9F8] rounded-2xl p-4 md:col-span-2">
              ...
              <p className="text-sm font-bold text-[#1A2E2A]">{owner.pincode}</p>
            </div>
          </div>
        </div>
        ...
        <div className="sticky bottom-0 px-6 py-4 bg-white border-t border-[#E8F0EE] rounded-b-3xl shrink-0 shadow-[0_-4px_20px_rgba(0,0,0,0.05)]">
          <div className="flex items-center gap-3">
            ...
            <button onClick={() => { if (onEdit) { onEdit(owner); onClose(); } }} className="flex-1 px-4 py-2.5 bg-[#26A69A] text-white rounded-xl hover:bg-[#1A8A7A] transition-all duration-300 text-sm font-medium shadow-lg shadow-[#26A69A]/30 hover:scale-[1.02]">
              <FiEdit className="inline mr-2" /> Edit
            </button>
            <button onClick={() => { if (onDelete) { onDelete(owner.id); } }} className="flex-1 px-4 py-2.5 bg-red-600 text-white rounded-xl hover:bg-red-700 transition-all duration-300 text-sm font-medium shadow-lg shadow-red-600/30 hover:scale-[1.02]">
...
// EDIT OWNER MODAL
// ============================================================
const EditOwnerModal = ({ owner, show, onClose, onSave, planTypes }) => {
  if (!owner || !show) return null;
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-fade-in">
      <div className="bg-white rounded-3xl max-w-4xl w-full max-h-[90vh] overflow-hidden shadow-2xl animate-slide-up border border-[#E8F0EE] flex flex-col">
        <div className="sticky top-0 bg-gradient-to-r from-[#00695C] to-[#26A69A] p-6 rounded-t-3xl z-10 shrink-0">
          ...
          <p className="text-white/80 text-sm">Update owner and subscription details</p>
        </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div className="bg-[#F5F9F8] rounded-2xl p-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                <div>
                  <label className="block text-xs font-medium text-[#5A7D78] mb-1">Owner ID</label>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
            ...
            <div className="bg-gradient-to-br from-purple-50 to-purple-100/50 rounded-2xl p-4 border border-purple-200">
              <div className="flex items-center gap-2 mb-2">
                ...
                <h4 className="text-xs font-bold text-purple-700 uppercase tracking-wider">Owners Using</h4>
              </div>
            ...
            <div className="bg-gradient-to-br from-amber-50 to-amber-100/50 rounded-2xl p-5 border border-amber-200">
              <div className="flex items-center gap-2 mb-3">
                ...
                <h4 className="text-sm font-bold text-amber-700 uppercase tracking-wider">
                  Owners on this Plan ({ownersOnPlan.length})
                </h4>
              </div>
              <div className="space-y-2 max-h-48 overflow-y-auto">
                {ownersOnPlan.map((owner) => (
                  <div key={owner.id} className="flex items-center gap-3 bg-white rounded-xl p-3 border border-amber-200 shadow-sm">
                    ...
                    <div className="min-w-0 flex-1">
                      <p className="text-sm font-bold text-[#1A2E2A] truncate">{owner.name}</p>
                      <p className="text-xs text-[#5A7D78] truncate">{owner.ownerId} • {owner.email}</p>
                    </div>
      ...
      <div className="bg-white rounded-3xl max-w-4xl w-full max-h-[90vh] overflow-hidden shadow-2xl animate-slide-up border border-[#E8F0EE] flex flex-col">
        <div className="sticky top-0 p-6 rounded-t-3xl z-10 shrink-0" style={{ background: previewTheme.gradient }}>
          ...
          <h2 className="text-2xl font-bold text-white">
            {isNew ? 'Add New Subscription Plan' : `Edit ${plan?.label || ''} Plan`}
          </h2>
          <p className="text-white/80 text-sm">
            {isNew ? 'Create a new subscription plan for property owners' : 'Update plan details, pricing, and features'}
          </p>
        </div>
        ...
        <div className="bg-gradient-to-r from-[#E8F4F2] to-[#F5F9F8] rounded-xl p-3 mb-3 border border-[#E8F0EE]">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <span className="text-xs font-semibold text-[#5A7D78] uppercase tracking-wider">Owners Using</span>
            </div>
...
const OwnerPlans = () => {
  ...
  const computeStats = useCallback((list, planTypesData) => {
    ...
    Object.keys(planTypesData).forEach(plan => {
      counts[plan] = list ? list.filter(o => o.subscriptionPlan === plan).length : 0;
    });
    setStats(counts);
    setPlanStats(counts);
  }, []);

  const generateMockOwners = useCallback((planNames) => {
    ...
    for (let i = 1; i <= 60; i++) {
      ...
      list.push({
        id: `own_${i}`, ownerId: `OWN-${String(i).padStart(4, '0')}`, name,
  ...
  useEffect(() => {
    try {
      ...
      const mockOwners = generateMockOwners(planNames);
      setOwners(mockOwners);
      setFilteredOwners(mockOwners);
      computeStats(mockOwners, DEFAULT_PLAN_TYPES);
    } catch (error) {
      console.error('Error generating mock owners:', error);
    }
  }, [generateMockOwners, computeStats]);
    ...
    try {
      ...
      if (searchQuery) {
        ...
        filtered = filtered.filter(o =>
          ...
          (o.ownerId && o.ownerId.toLowerCase().includes(query)) ||
          (o.email && o.email.toLowerCase().includes(query)) ||
          (o.phone && o.phone.toLowerCase().includes(query)) ||
          (o.propertyType && o.propertyType.toLowerCase().includes(query)) ||
          (o.subscriptionPlan && o.subscriptionPlan.toLowerCase().includes(query)) ||
          (o.state && o.state.toLowerCase().includes(query)) ||
          (o.city && o.city.toLowerCase().includes(query))
        );
      }
      if (activePlan !== 'all') filtered = filtered.filter(o => o.subscriptionPlan === activePlan);
      if (activePropertyType !== 'all') filtered = filtered.filter(o => o.propertyType === activePropertyType);
      ...
      console.error('Error filtering owners:', error);
    }
  }, [owners, searchQuery, activePlan, activePropertyType, sortField, sortDirection]);

  useEffect(() => { filterOwners(); }, [filterOwners]);
  ...
  const handleSelectAll = useCallback(() => {
    if (selectedOwners.length === paginatedOwners.length && paginatedOwners.length > 0) setSelectedOwners([]);
    else setSelectedOwners(paginatedOwners.map(o => o.id));
  ...
  const handleViewOwner = useCallback((owner) => { setViewingOwner(owner); setShowViewModal(true); }, []);
  const handleEditOwner = useCallback((owner) => { setEditingOwner(owner); setShowEditModal(true); }, []);

  const handleSaveOwner = useCallback((updatedOwner) => {
    setOwners(prev => {
      const updated = prev.map(o => o.id === updatedOwner.id ? updatedOwner : o);
      computeStats(updated, planTypes);
      return updated;
    });
    setToast({ message: `Owner "${updatedOwner.name}" updated successfully`, type: 'success' });
  ...
  const handleDeletePlan = useCallback((planKey) => {
    ...
    if (!plan) return;
    const ownerCount = owners.filter(o => o.subscriptionPlan === planKey).length;

    setConfirmationModal({
      isOpen: true,
      title: 'Delete Subscription Plan',
      message: `Are you sure you want to delete the "${plan.label}" plan?${ownerCount > 0 ? ` ${ownerCount} owner(s) are currently on this plan and will be moved to Free plan.` : ''}`,
      ...
      onConfirm: () => {
        ...
        setTimeout(() => {
          ...
          setOwners(prev => {
            ...
            const fallbackPlan = remainingPlans.includes('Free') ? 'Free' : remainingPlans[0] || 'Free';
            const updated = prev.map(o => o.subscriptionPlan === planKey ? { ...o, subscriptionPlan: fallbackPlan } : o);
  ...
  const handleDeleteOwner = useCallback((ownerId) => {
    ...
    setConfirmationModal({
      isOpen: true,
      title: 'Delete Owner',
      message: `Are you sure you want to delete owner "${owner.name}" (${owner.ownerId})?`,
      ...
      onConfirm: () => {
        ...
        setTimeout(() => {
          ...
          setToast({ message: `Deleted owner "${owner.name}"`, type: 'warning' });
        }, 700);
      },
      onCancel: () => setConfirmationModal(prev => ({ ...prev, isOpen: false }))
    });
  }, [owners, computeStats, planTypes]);
  ...
  const handleRefresh = useCallback(() => {
    ...
    setTimeout(() => {
      try {
        ...
        setOwners(mockOwners);
        setFilteredOwners(mockOwners);
        computeStats(mockOwners, planTypes);
  ...
  }, [generateMockOwners, planTypes, computeStats]);
  ...
  const handleExport = useCallback(() => {
    ...
    try {
      const data = filteredOwners.map(o => ({
        'Owner ID': o.ownerId || '', 'Name': o.name || '', 'Mail ID': o.email || '',
        'Phone Number': o.phone || '', 'Property Type': o.propertyType || '',
        'Subscription Plan': planTypes[o.subscriptionPlan]?.label || o.subscriptionPlan || '',
        'Street': o.street || '', 'Area / Locality': o.area || '', 'City': o.city || '',
        'District': o.district || '', 'State': o.state || '', 'Country': o.country || '', 'Pincode': o.pincode || ''
      }));
      ...
      a.href = url;
      a.download = `owner_plans_${new Date().toISOString().split('T')[0]}.csv`;
      a.click();
      window.URL.revokeObjectURL(url);
      setToast({ message: `${filteredOwners.length} records exported successfully`, type: 'success' });
    } catch (error) {
      setToast({ message: 'Error exporting data', type: 'error' });
    }
  }, [filteredOwners, planTypes]);

  const handleBulkDelete = useCallback(() => {
    if (selectedOwners.length === 0) {
      setToast({ message: 'Please select owners first', type: 'warning' });
      return;
    }
    setConfirmationModal({
      isOpen: true,
      title: 'Delete Selected Owners',
      message: `Are you sure you want to delete ${selectedOwners.length} selected owner(s)?`,
      confirmText: 'Delete All', cancelText: 'Cancel', type: 'danger',
      onConfirm: () => {
        setActionLoading('bulk-delete');
        setTimeout(() => {
          const selectedIds = new Set(selectedOwners);
          const count = owners.filter(o => selectedIds.has(o.id)).length;
          const updated = owners.filter(o => !selectedIds.has(o.id));
          setOwners(updated);
          computeStats(updated, planTypes);
          setSelectedOwners([]);
          setActionLoading(null);
          setToast({ message: `${count} owner(s) deleted`, type: 'warning' });
        }, 800);
      },
      onCancel: () => setConfirmationModal(prev => ({ ...prev, isOpen: false }))
    });
  }, [selectedOwners, owners, computeStats, planTypes]);
  ...
  // Get owners for the viewing plan
  const ownersOnViewingPlan = viewingPlan
    ? owners.filter(o => o.subscriptionPlan === viewingPlan.name)
    : [];
  ...
  return (
    <div className="space-y-6 p-4 lg:p-6 bg-[#F8FAF9] min-h-screen">
      ...
      {showViewModal && viewingOwner && (
        <ViewOwnerDetailModal owner={viewingOwner} show={showViewModal}
          onClose={() => { setShowViewModal(false); setViewingOwner(null); }}
          onEdit={handleEditOwner} onDelete={handleDeleteOwner} planTypes={planTypes} />
      )}

      {showEditModal && editingOwner && (
        <EditOwnerModal owner={editingOwner} show={showEditModal}
          onClose={() => { setShowEditModal(false); setEditingOwner(null); }}
          onSave={handleSaveOwner} planTypes={planTypes} />
      )}
      ...
      {showViewPlanModal && viewingPlan && (
        <ViewPlanModal plan={viewingPlan} show={showViewPlanModal}
          ...
          ownerCount={ownersOnViewingPlan.length}
          ownersOnPlan={ownersOnViewingPlan} />
      )}
        ...
        <div className="flex flex-col lg:flex-row items-start lg:items-center justify-between gap-4">
          <div>
            ...
            <p className="text-sm text-[#5A7D78] flex items-center gap-2 flex-wrap">
              <span>Manage subscription plans and property owners</span>
        ...
        <div className="relative animate-slide-in">
          <div className="bg-white rounded-2xl p-4 border border-[#E8F0EE] shadow-sm">
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-3">
              <StatCard icon={<FiUsers className="text-white text-sm" />} title="Total Owners" value={stats.total}
                      ...
                      <div key={planKey}
                        ...
                        <div className="flex items-center gap-3 flex-wrap">
                          ...
                          <span className="text-xs font-bold text-purple-700 bg-purple-100 px-2 py-1 rounded-full flex items-center gap-1">
                            <FiUsers className="text-[10px]" /> {planStats[planKey] || 0} owners
                          </span>
      ...
      <div className="relative bg-white rounded-2xl p-4 shadow-sm border border-[#E8F0EE] hover:shadow-md transition-all duration-300">
        <div className="flex flex-col lg:flex-row items-start lg:items-center gap-4">
          <div className="flex-1 w-full relative">
            ...
            <input ref={searchInputRef} type="text" placeholder="Search owners by name, ID, email, phone, city..."
        ...
        {selectedOwners.length > 0 && (
          <div className="mt-4 pt-4 border-t border-[#E8F0EE] flex flex-wrap items-center justify-between gap-3 animate-slide-in">
            ...
            <div className="flex flex-wrap items-center gap-2">
              ...
              <button onClick={() => setSelectedOwners([])}
      ...
      <div className="relative">
        {loading ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-4 gap-4">
            {paginatedOwners.map((owner, index) => {
              ...
              const planConfig = planTypes[owner.subscriptionPlan] || { theme: FALLBACK_THEME, label: owner.subscriptionPlan, icon: FiPackage };
              const theme = planConfig.theme || FALLBACK_THEME;
              const PlanIcon = planConfig.icon || FiPackage;
              const propTypeConfig = PROPERTY_TYPE_CONFIG[owner.propertyType] || PROPERTY_TYPE_CONFIG['Individual'];
              const PropTypeIcon = propTypeConfig.icon;
                ...
                <div key={owner.id}
                  ...
                  <div className="flex items-start justify-between mb-2 gap-1">
                    <div className="flex items-center gap-2 min-w-0">
                      <input type="checkbox" checked={isSelected} onChange={() => handleSelectOwner(owner.id)}
                      ...
                      <div className="min-w-0">
                        <h3 className="font-bold text-sm text-[#1A2E2A] truncate">{owner.name}</h3>
                        <div className="flex items-center gap-1.5 mt-0.5 flex-wrap">
                          <p className="text-[10px] font-medium text-[#5A7D78]">{owner.ownerId}</p>
                  ...
                  <div className="space-y-1">
                    ...
                    <div className="flex items-center gap-2 text-[11px] text-[#5A7D78]">
                      ...
                      <span className="truncate font-medium">{owner.phone}</span>
                    </div>
                    ...
                    <div className="flex items-center gap-2 text-[11px] text-[#5A7D78]">
                      ...
                      <span className="truncate font-medium">{owner.area}, {owner.city}</span>
                    </div>
                  </div>

                  <div className="flex flex-wrap gap-1 mt-2.5 pt-2.5 border-t border-[#E8F0EE]">
                    <button type="button" onClick={() => handleViewOwner(owner)}
                    ...
                    <button type="button" onClick={() => handleDeleteOwner(owner.id)} disabled={actionLoading === owner.id}
                      ...
                      {actionLoading === owner.id ? <FiRefreshCw className="text-[10px] animate-spin" /> : <FiTrash2 className="text-[10px]" />}
                      Delete
                    </button>
                  </div>
                </div>
          ...
          <div className="bg-white rounded-2xl border border-[#E8F0EE] shadow-sm overflow-hidden">
            <div className="grid grid-cols-12 gap-1 items-center px-4 py-3 bg-[#F5F9F8] border-b border-[#E8F0EE] text-xs font-semibold text-[#5A7D78] uppercase tracking-wider">
              <div className="col-span-2 flex items-center gap-2 min-w-0">
                <input type="checkbox" checked={selectedOwners.length === paginatedOwners.length && paginatedOwners.length > 0}
              ...
              return (
                <div key={owner.id}
                  ...
                  <div className="col-span-2 flex items-center gap-2 min-w-0">
                    ...
                    <span className="text-xs font-bold text-[#00695C] truncate">{owner.ownerId}</span>
                  </div>
                  <div className="col-span-2 min-w-0">
                    <p className="font-bold text-sm text-[#1A2E2A] truncate">{owner.name}</p>
                  </div>
                  <div className="col-span-2 min-w-0 text-xs font-medium text-[#5A7D78] truncate">{owner.email}</div>
                  <div className="col-span-1 min-w-0 text-xs font-medium text-[#5A7D78] truncate">{owner.phone}</div>
                  ...
                  <div className="col-span-1 min-w-0 text-xs font-medium text-[#5A7D78] truncate">{owner.city}</div>
                  <div className="col-span-1 min-w-0 text-xs font-medium text-[#5A7D78] truncate">{owner.state}</div>
                  <div className="col-span-1 min-w-0 flex items-center justify-end gap-1 flex-nowrap">
                    ...
                    <button type="button" onClick={() => handleDeleteOwner(owner.id)} disabled={actionLoading === owner.id}
                      ...
                      {actionLoading === owner.id ? <FiRefreshCw className="text-[9px] animate-spin" /> : <FiTrash2 className="text-[15px]" />}
                    </button>
                  </div>
                </div>
              );
        ...
        {paginatedOwners.length === 0 && !loading && (
          <div className="flex flex-col items-center justify-center py-20 text-center bg-white rounded-2xl border border-[#E8F0EE]">
            ...
            <h3 className="text-xl font-bold text-[#1A2E2A]">No owners found</h3>
            <p className="text-sm text-[#5A7D78] mt-1">
              {filterCount > 0 ? 'Try adjusting your search or filter criteria' : 'No property owners have been added yet'}
            </p>
            {filterCount > 0 && (
              <button onClick={clearAllFilters}
        ...
        <div className="flex flex-wrap items-center justify-between bg-white rounded-2xl px-4 py-3 border border-[#E8F0EE] shadow-sm gap-3">
          <div className="flex items-center gap-2 text-sm text-[#5A7D78] flex-wrap">
            <span className="font-medium">
              Showing {(currentPage - 1) * pageSize + 1} to {Math.min(currentPage * pageSize, filteredOwners.length)} of {filteredOwners.length} owners
            </span>

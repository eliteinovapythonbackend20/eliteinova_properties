// src/components/dashboard/properties/Commercial/PetrolBunk.jsx
...
import { FaCity, FaGasPump } from 'react-icons/fa';
...
// ============================================================
// FIXED PROPERTY TYPE — this page only handles Petrol Bunk / Fuel Station
// ============================================================
const PETROL_BUNK_TYPE = {
  icon: FaGasPump,
      ...
      <div className="bg-white rounded-3xl max-w-4xl w-full max-h-[90vh] overflow-hidden shadow-2xl animate-slide-up border border-[#E8F0EE] flex flex-col">
        ...
        <div className="sticky top-0 bg-gradient-to-r from-[#00695C] to-[#26A69A] p-6 rounded-t-3xl z-10 shrink-0">
          ...
          <div className="flex items-center gap-2 flex-wrap">
            <span className={`px-3 py-1 rounded-full text-xs font-semibold flex items-center gap-1.5 ${PETROL_BUNK_TYPE.bg} ${PETROL_BUNK_TYPE.text} border ${PETROL_BUNK_TYPE.border}`}>
              <FaGasPump className="text-xs" /> {PETROL_BUNK_TYPE.label}
            </span>
          </div>
        </div>
        ...
        <div className="flex-1 overflow-y-auto p-6 bg-white">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div className="bg-[#F5F9F8] rounded-2xl p-4">
              ...
              <p className="text-sm font-bold text-[#1A2E2A]">{PETROL_BUNK_TYPE.label}</p>
            </div>

            <div className="bg-[#F5F9F8] rounded-2xl p-4">
              <div className="flex items-center gap-2 mb-1">
                <FaGasPump className="text-[#00695C] text-sm" />
...
const EditPropertyModal = ({ property, show, onClose, onSave }) => {
  ...
  const handleSubmit = (e) => {
    ...
    setLoading(true);
    setTimeout(() => {
      onSave({ ...property, ...formData, propertyType: PETROL_BUNK_TYPE.label });
      setLoading(false);
      onClose();
    }, 700);
  };
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm animate-fade-in">
      <div className="bg-white rounded-3xl max-w-4xl w-full max-h-[90vh] overflow-hidden shadow-2xl animate-slide-up border border-[#E8F0EE] flex flex-col">
        ...
        <div className="sticky top-0 bg-gradient-to-r from-[#00695C] to-[#26A69A] p-6 rounded-t-3xl z-10 shrink-0">
          ...
          <h2 className="text-2xl font-bold text-white">Edit Petrol Bunk / Fuel Station</h2>
          <p className="text-white/80 text-sm">Update petrol bunk / fuel station property details</p>
        </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div className="bg-[#F5F9F8] rounded-2xl p-4">
              <h3 className="text-xs font-semibold text-[#5A7D78] uppercase tracking-wider mb-3 flex items-center gap-2">
                <FaGasPump className="text-[#00695C]" />
                Basic Details
              </h3>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                ...
                <div>
                  ...
                  <input
                    type="text" value={PETROL_BUNK_TYPE.label} disabled
...
const PetrolBunk = () => {
  ...
  const mapCard = useCallback((card) => ({
    ...
    propertyType: card.propertyType || PETROL_BUNK_TYPE.label,
  ...
  const {
    ...
    fixedPropertyType: PETROL_BUNK_TYPE.filterValue,
    mapCard,
  ...
  const handleExport = useCallback(async () => {
    ...
    try {
      ...
      a.href = url;
      a.download = `petrol_bunk_fuel_station_${new Date().toISOString().split('T')[0]}.csv`;
      a.click();
        ...
        <div className="flex flex-col lg:flex-row items-start lg:items-center justify-between gap-4">
          <div>
            ...
            <p className="text-sm text-[#5A7D78] flex items-center gap-2 flex-wrap">
              <span>All Petrol Bunk / Fuel Station listings across Buy, Rent &amp; Lease</span>
        ...
        <div className="relative animate-slide-in">
          <div className="bg-white rounded-2xl p-4 border border-[#E8F0EE] shadow-sm">
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
              <StatCard
                icon={<FaGasPump className="text-white text-sm" />}
                ...
                >
                  ...
                  <div className="space-y-1">
                    <div className="flex items-center gap-2 text-[11px] text-[#5A7D78]">
                      <FaGasPump className="text-[#00695C] flex-shrink-0" />
                      <span className="truncate font-semibold text-[#1A2E2A]">{PETROL_BUNK_TYPE.label}</span>
                    </div>
                    <div className="flex items-center gap-2 text-[11px] text-[#5A7D78]">
        ...
        {paginatedProperties.length === 0 && !loading && (
          <div className="flex flex-col items-center justify-center py-20 text-center bg-white rounded-2xl border border-[#E8F0EE]">
            <div className="w-24 h-24 rounded-full bg-[#F5F9F8] flex items-center justify-center mb-4 animate-float">
              <FaGasPump className="text-4xl text-[#B5C9C5]" />
            </div>
            <h3 className="text-xl font-bold text-[#1A2E2A]">No properties found</h3>
            <p className="text-sm text-[#5A7D78] mt-1">
              {filterCount > 0 ? 'Try adjusting your search or filter criteria' : 'No petrol bunk / fuel station listings have been added yet'}
            </p>
            {filterCount > 0 && (
              <button
                onClick={clearAllFilters}

// src/components/dashboard/properties/LandPlots/ResidentialLandPlots.jsx
...
import {
  ...
  FiMap, FiActivity, FiShield, FiLayers, FiKey
} from 'react-icons/fi';
import { FaCity, FaHome, FaTractor, FaBuilding } from 'react-icons/fa';

// ============================================================
// FIXED MAIN PROPERTY TYPE — this page only handles Residential Land / Plots
// ============================================================
const MAIN_TYPE_LABEL = 'Residential Land / Plots';
...
const SUB_TYPES = {
  ...
  'DtcpCmdaApprovedPlot': {
    icon: FiCheckCircle,
    color: 'from-blue-600 to-blue-400',
    bg: 'bg-blue-50',
    text: 'text-blue-700',
    border: 'border-blue-200',
    label: 'DTCP & CMDA Approved Plot'
  },
  'GatedCommunityPlot': {
    icon: FiShield,
...
const ViewPropertyDetailModal = ({ property, show, onClose, onEdit, onDelete }) => {
  ...
  const subConfig = SUB_TYPES[property.propertySubType] || SUB_TYPES['ResidentialPlot'];
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm animate-fade-in">
      <div className="bg-white rounded-3xl max-w-4xl w-full max-h-[90vh] overflow-hidden shadow-2xl animate-slide-up border border-[#E8F0EE] flex flex-col">
        ...
        <div className="sticky top-0 bg-gradient-to-r from-[#00695C] to-[#26A69A] p-6 rounded-t-3xl z-10 shrink-0">
          ...
          <h2 className="text-2xl font-bold text-white">Edit Residential Land / Plot</h2>
          <p className="text-white/80 text-sm">Update residential land / plot property details</p>
        </div>
...
const ResidentialLandPlots = () => {
  ...
  const generateMockProperties = useCallback(() => {
    const propertyTitles = [
      'Green Meadows Residential Plot', 'Sunrise DTCP Approved Plot', 'Golden Gate Gated Community Plot',
      'Palm Grove Villa Plot', 'Countryside Farm House Plot', 'Union Common Plot',
      'Independent Living House Plot', 'Twin Palms Duplex House Plot', 'Row House Enclave Plot',
      'Silverline Residential Plot', 'Riverside CMDA Approved Plot', 'Fortune Gated Community Plot',
      'Emerald Villa Plot', 'Meadow Farm House Plot', 'Heritage Common Plot',
      'Prime Independent House Plot', 'Skyline Duplex House Plot', 'Legacy Row House Plot'
    ];
    ...
    for (let i = 1; i <= 60; i++) {
      ...
      propertiesList.push({
        id: `rl_${i}`,
        propertyId: `RL-${String(i).padStart(4, '0')}`,
        propertyTitle: propertyTitles[Math.floor(Math.random() * propertyTitles.length)],
        propertyType: MAIN_TYPE_LABEL,
        propertySubType,
        listingType,
        description: `A well-located ${SUB_TYPES[propertySubType].label.toLowerCase()} under residential land / plots, available for ${listingType.toLowerCase()}.`,
  ...
  const handleExport = useCallback(() => {
    ...
    try {
      ...
      a.href = url;
      a.download = `residential_land_plots_${new Date().toISOString().split('T')[0]}.csv`;
      a.click();
        ...
        <div className="flex flex-col lg:flex-row items-start lg:items-center justify-between gap-4">
          <div>
            ...
            <p className="text-sm text-[#5A7D78] flex items-center gap-2 flex-wrap">
              <span>All Residential Land / Plots sub types across Buy, Rent &amp; Lease</span>
        ...
        {paginatedProperties.length === 0 && !loading && (
          <div className="flex flex-col items-center justify-center py-20 text-center bg-white rounded-2xl border border-[#E8F0EE]">
            ...
            <p className="text-sm text-[#5A7D78] mt-1">
              {filterCount > 0 ? 'Try adjusting your search or filter criteria' : 'No residential land / plot listings have been added yet'}
            </p>
            {filterCount > 0 && (
              <button
                onClick={clearAllFilters}
...
export default ResidentialLandPlots;
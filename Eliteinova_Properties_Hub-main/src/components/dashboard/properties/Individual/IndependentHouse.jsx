// src/components/dashboard/properties/Individual/IndependentHouse.jsx
...
// ============================================================
// FIXED PROPERTY TYPE — this page only handles Independent House
// ============================================================
const HOUSE_TYPE = {
  icon: FaHome,
  ...
  label: 'Independent House (Individual)',
  // The real property_type value the posting forms write to the DB (see
  // e.g. Forms/Owner/Individual/IndSellForm.jsx) - kept separate from the
  // decorative `label` above so the backend filter always matches real
  // data regardless of what's shown on screen.
  filterValue: 'Independent House'
};
      ...
      <div className="bg-white rounded-3xl max-w-4xl w-full max-h-[90vh] overflow-hidden shadow-2xl animate-slide-up border border-[#E8F0EE] flex flex-col">
        ...
        <div className="sticky top-0 bg-gradient-to-r from-[#00695C] to-[#26A69A] p-6 rounded-t-3xl z-10 shrink-0">
          ...
          <div className="flex items-center gap-2 flex-wrap">
            <span className={`px-3 py-1 rounded-full text-xs font-semibold flex items-center gap-1.5 ${HOUSE_TYPE.bg} ${HOUSE_TYPE.text} border ${HOUSE_TYPE.border}`}>
              <FaHome className="text-xs" /> {HOUSE_TYPE.label}
            </span>
          </div>
        </div>
        ...
        <div className="flex-1 overflow-y-auto p-6 bg-white">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div className="bg-[#F5F9F8] rounded-2xl p-4">
              ...
              <p className="text-sm font-bold text-[#1A2E2A]">{HOUSE_TYPE.label}</p>
            </div>
...
const EditPropertyModal = ({ property, show, onClose, onSave }) => {
  ...
  const handleSubmit = (e) => {
    ...
    setLoading(true);
    setTimeout(() => {
      onSave({ ...property, ...formData, propertyType: HOUSE_TYPE.label });
      setLoading(false);
      onClose();
    }, 700);
  };
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm animate-fade-in">
      <div className="bg-white rounded-3xl max-w-4xl w-full max-h-[90vh] overflow-hidden shadow-2xl animate-slide-up border border-[#E8F0EE] flex flex-col">
        ...
        <div className="sticky top-0 bg-gradient-to-r from-[#00695C] to-[#26A69A] p-6 rounded-t-3xl z-10 shrink-0">
          ...
          <h2 className="text-2xl font-bold text-white">Edit Independent House</h2>
            ...
            <div className="bg-[#F5F9F8] rounded-2xl p-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                ...
                <div>
                  ...
                  <input
                    type="text" value={HOUSE_TYPE.label} disabled
...
const IndependentHouse = () => {
  ...
  const mapCard = useCallback((card) => ({
    ...
    propertyType: card.propertyType || HOUSE_TYPE.label,
  ...
  const {
    ...
    fixedPropertyType: HOUSE_TYPE.filterValue,
    mapCard,
  ...
  const handleExport = useCallback(async () => {
    ...
    try {
      ...
      a.href = url;
      a.download = `independent_house_${new Date().toISOString().split('T')[0]}.csv`;
      a.click();
                ...
                >
                  ...
                  <div className="space-y-1">
                    <div className="flex items-center gap-2 text-[11px] text-[#5A7D78]">
                      ...
                      <span className="truncate font-semibold text-[#1A2E2A]">{HOUSE_TYPE.label}</span>
                    </div>
                    <div className="flex items-center gap-2 text-[11px] text-[#5A7D78]">
        ...
        {paginatedProperties.length === 0 && !loading && (
          <div className="flex flex-col items-center justify-center py-20 text-center bg-white rounded-2xl border border-[#E8F0EE]">
            ...
            <p className="text-sm text-[#5A7D78] mt-1">
              {filterCount > 0 ? 'Try adjusting your search or filter criteria' : 'No independent house listings have been added yet'}
            </p>
            {filterCount > 0 && (
              <button
                onClick={clearAllFilters}

// src/components/dashboard/properties/Commercial/CommercialLandPlot.jsx
...
// ============================================================
// FIXED PROPERTY TYPE — this page only handles Commercial Land Plot
// ============================================================
const COMMERCIAL_LAND_PLOT_TYPE = {
  icon: FaMapSigns,
      ...
      <div className="bg-white rounded-3xl max-w-4xl w-full max-h-[90vh] overflow-hidden shadow-2xl animate-slide-up border border-[#E8F0EE] flex flex-col">
        ...
        <div className="sticky top-0 bg-gradient-to-r from-[#00695C] to-[#26A69A] p-6 rounded-t-3xl z-10 shrink-0">
          ...
          <div className="flex items-center gap-2 flex-wrap">
            <span className={`px-3 py-1 rounded-full text-xs font-semibold flex items-center gap-1.5 ${COMMERCIAL_LAND_PLOT_TYPE.bg} ${COMMERCIAL_LAND_PLOT_TYPE.text} border ${COMMERCIAL_LAND_PLOT_TYPE.border}`}>
              <FaMapSigns className="text-xs" /> {COMMERCIAL_LAND_PLOT_TYPE.label}
            </span>
          </div>
        </div>
        ...
        <div className="flex-1 overflow-y-auto p-6 bg-white">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div className="bg-[#F5F9F8] rounded-2xl p-4">
              ...
              <p className="text-sm font-bold text-[#1A2E2A]">{COMMERCIAL_LAND_PLOT_TYPE.label}</p>
            </div>
...
const EditPropertyModal = ({ property, show, onClose, onSave }) => {
  ...
  const handleSubmit = (e) => {
    ...
    setLoading(true);
    setTimeout(() => {
      onSave({ ...property, ...formData, propertyType: COMMERCIAL_LAND_PLOT_TYPE.label });
      setLoading(false);
      onClose();
    }, 700);
  };
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm animate-fade-in">
      <div className="bg-white rounded-3xl max-w-4xl w-full max-h-[90vh] overflow-hidden shadow-2xl animate-slide-up border border-[#E8F0EE] flex flex-col">
        ...
        <div className="sticky top-0 bg-gradient-to-r from-[#00695C] to-[#26A69A] p-6 rounded-t-3xl z-10 shrink-0">
          ...
          <h2 className="text-2xl font-bold text-white">Edit Commercial Land Plot</h2>
          <p className="text-white/80 text-sm">Update commercial land plot details</p>
        </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div className="bg-[#F5F9F8] rounded-2xl p-4">
              <h3 className="text-xs font-semibold text-[#5A7D78] uppercase tracking-wider mb-3 flex items-center gap-2">
                <FaMapSigns className="text-[#00695C]" />
                Basic Details
              </h3>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                ...
                <div>
                  ...
                  <input
                    type="text" value={COMMERCIAL_LAND_PLOT_TYPE.label} disabled
                ...
                <div className="md:col-span-2">
                  ...
                  <textarea
                    ...
                    placeholder="Describe the commercial land plot..."
                  />
                </div>
              </div>
            </div>
...
const CommercialLandPlot = () => {
  ...
  const mapCard = useCallback((card) => ({
    ...
    propertyType: card.propertyType || COMMERCIAL_LAND_PLOT_TYPE.label,
  ...
  const {
    ...
    fixedPropertyType: COMMERCIAL_LAND_PLOT_TYPE.filterValue,
    mapCard,
  ...
  const handleExport = useCallback(async () => {
    ...
    try {
      ...
      a.href = url;
      a.download = `commercial_land_plot_${new Date().toISOString().split('T')[0]}.csv`;
      a.click();
        ...
        <div className="flex flex-col lg:flex-row items-start lg:items-center justify-between gap-4">
          <div>
            ...
            <p className="text-sm text-[#5A7D78] flex items-center gap-2 flex-wrap">
              <span>All Commercial Land Plot listings across Buy, Rent &amp; Lease</span>
        ...
        <div className="relative animate-slide-in">
          <div className="bg-white rounded-2xl p-4 border border-[#E8F0EE] shadow-sm">
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
              <StatCard
                icon={<FaMapSigns className="text-white text-sm" />}
                ...
                >
                  ...
                  <div className="space-y-1">
                    <div className="flex items-center gap-2 text-[11px] text-[#5A7D78]">
                      <FaMapSigns className="text-[#00695C] flex-shrink-0" />
                      <span className="truncate font-semibold text-[#1A2E2A]">{COMMERCIAL_LAND_PLOT_TYPE.label}</span>
                    </div>
                    <div className="flex items-center gap-2 text-[11px] text-[#5A7D78]">
        ...
        {paginatedProperties.length === 0 && !loading && (
          <div className="flex flex-col items-center justify-center py-20 text-center bg-white rounded-2xl border border-[#E8F0EE]">
            <div className="w-24 h-24 rounded-full bg-[#F5F9F8] flex items-center justify-center mb-4 animate-float">
              <FaMapSigns className="text-4xl text-[#B5C9C5]" />
            </div>
            <h3 className="text-xl font-bold text-[#1A2E2A]">No properties found</h3>
            <p className="text-sm text-[#5A7D78] mt-1">
              {filterCount > 0 ? 'Try adjusting your search or filter criteria' : 'No commercial land plot listings have been added yet'}
            </p>
            {filterCount > 0 && (
              <button
                onClick={clearAllFilters}
...
export default CommercialLandPlot;
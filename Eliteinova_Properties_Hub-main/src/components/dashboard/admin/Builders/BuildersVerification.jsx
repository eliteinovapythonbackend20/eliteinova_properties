// src/components/dashboard/admin/Builders/BuildersVerification.jsx
...
import { 
  ...
  MdVerified, MdBusiness, MdCorporateFare, MdConstruction 
} from 'react-icons/md';
...
// ============ VIEW BUILDER / VERIFICATION MODAL ============
const BuilderVerificationModal = ({
  builder,
  show,
  onClose,
  onOpenDocument,
  ...
  const sections = [
    {
      ...
      subtitle: builder.companyDoc.number ? `Reg No: ${builder.companyDoc.number}` : 'Not provided',
      icon: <MdBusiness className="text-sm" />,
      doc: builder.companyDoc,
    },
    {
      key: 'gst',
      title: 'GST Verification',
      subtitle: builder.gstDoc.number ? `GSTIN: ${builder.gstDoc.number}` : 'Not provided',
    ...
    },
    {
      ...
      subtitle: builder.reraDoc.number ? `RERA No: ${builder.reraDoc.number}` : 'Not provided',
        ...
        <div className="relative bg-gradient-to-r from-[#00695C] to-[#26A69A] p-6 shrink-0">
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="flex-1 min-w-0">
              <h2 className="text-xl font-bold text-white truncate">{builder.name}</h2>
              <p className="text-white/80 text-sm truncate">{builder.companyName} · {builder.city}, {builder.state}</p>
            </div>
          </div>
          <div className="flex items-center gap-2 mt-3 flex-wrap">
            <span className={`text-[10px] px-2.5 py-1 rounded-full font-semibold ${
              builder.overallStatus === 'approved' ? 'bg-emerald-100 text-emerald-700' :
              builder.overallStatus === 'rejected' ? 'bg-red-100 text-red-700' :
              'bg-amber-100 text-amber-700'
            }`}>
              {builder.overallStatus === 'approved' ? 'Fully Verified' : builder.overallStatus === 'rejected' ? 'Has Rejections' : 'Verification Pending'}
            </span>
            <span className="text-[10px] px-2.5 py-1 rounded-full font-semibold bg-white/20 text-white">
              Submitted {new Date(builder.submittedDate).toLocaleDateString('en-IN', { day: 'numeric', month: 'short', year: 'numeric' })}
            </span>
          </div>
        </div>
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-5">
          ...
          <div className="flex items-center justify-between p-3 bg-[#F5F9F8] rounded-xl">
            <div className="flex items-center gap-3 min-w-0">
              ...
              <div className="min-w-0">
                <p className="text-sm font-semibold text-[#1A2E2A] truncate">{builder.name}</p>
                <p className="text-xs text-[#5A7D78] truncate">Builder · {builder.email}</p>
              </div>
            </div>
            <button
              onClick={() => onViewBuilderProfile(builder.id)}
          ...
          <div className="border border-[#E8F0EE] rounded-2xl p-4">
            <div className="flex items-center gap-2 mb-3">
              ...
              <div>
                <h4 className="text-sm font-semibold text-[#1A2E2A]">Company Documents</h4>
                <p className="text-[11px] text-[#5A7D78]">Supporting documents & certificates</p>
              </div>
            </div>

            <div className="space-y-2.5">
              {builder.companyDocs.map(doc => (
...
const BuildersVerification = () => {
  ...
  const [activeTab, setActiveTab] = useState('all'); // all | company | gst | rera | documents
  ...
  const generateMockBuilders = useCallback(() => {
    ...
    const names = [
      'Shree Construction', 'BuildWell Developers', 'Green Valley Builders', 'Urban Homes', 'Premier Estates',
      'Golden Gate Constructions', 'Maple Housing', 'Sapphire Builders', 'Crystal Developers', 'Silver Oak Group',
      'Heritage Constructions', 'Fortune Builders', 'Apex Developers', 'Elite Housing', 'Royal Estates',
      'Sunrise Constructions', 'Skyline Developers', 'Pacific Builders', 'Metro Housing', 'Grand Constructions'
    ];
    const companies = [
      'Shree Construction Pvt Ltd', 'BuildWell Developers LLP', 'Green Valley Builders', 'Urban Homes Group',
      'Premier Estates Ltd', 'Golden Gate Constructions', 'Maple Housing Corp', 'Sapphire Builders Pvt Ltd'
    ];
    ...
    for (let i = 1; i <= 38; i++) {
      ...
      const companyStatus = pick(statuses);
      const gstStatus = pick(statuses);
      const reraStatus = pick(statuses);
      const doc1Status = pick(statuses);
      const doc2Status = pick(statuses);

      const allStatuses = [companyStatus, gstStatus, reraStatus, doc1Status, doc2Status];
      ...
      list.push({
        ...
        companyDoc: {
          ...
          category: 'company',
          number: `CIN-${String(100000 + i)}`,
          url: `https://picsum.photos/seed/company${i}/700/500`,
        ...
        gstDoc: {
          ...
          category: 'gst',
          number: `22ABCDE${String(1000 + i)}1Z5`,
          url: `https://picsum.photos/seed/gst${i}/700/500`,
          status: gstStatus,
          builderName: name,
        },
        reraDoc: {
          ...
          url: `https://picsum.photos/seed/rera_builder${i}/700/500`,
          status: reraStatus,
          builderName: name,
        },
        companyDocs: [
          {
            ...
            category: 'documents',
            number: `COI-${String(10000 + i)}`,
            url: `https://picsum.photos/seed/coi${i}/700/500`,
            status: doc1Status,
            builderName: name,
          },
          {
            ...
            category: 'documents',
            number: `MOA-${String(10000 + i)}`,
            url: `https://picsum.photos/seed/moa${i}/700/500`,
            status: doc2Status,
            builderName: name,
          },
        ],
  ...
  const updateStats = useCallback((list) => {
    setStats({
      ...
      pendingCompany: list.filter(a => a.companyDoc.status === 'pending').length,
      pendingGST: list.filter(a => a.gstDoc.status === 'pending').length,
      pendingRERA: list.filter(a => a.reraDoc.status === 'pending').length,
      pendingDocs: list.filter(a => a.companyDocs.some(d => d.status === 'pending')).length,
  ...
  const filterBuilders = useCallback(() => {
    ...
    if (activeTab === 'company' && statusFilter !== 'all') {
      filtered = filtered.filter(a => a.companyDoc.status === statusFilter);
    } else if (activeTab === 'gst' && statusFilter !== 'all') {
      filtered = filtered.filter(a => a.gstDoc.status === statusFilter);
    } else if (activeTab === 'rera' && statusFilter !== 'all') {
      filtered = filtered.filter(a => a.reraDoc.status === statusFilter);
    } else if (activeTab === 'documents' && statusFilter !== 'all') {
      filtered = filtered.filter(a => a.companyDocs.some(d => d.status === statusFilter));
    ...
    setCurrentPage(1);
  }, [builders, searchQuery, activeTab, statusFilter]);
  ...
  const paginatedBuilders = useMemo(() =>
    filteredBuilders.slice((currentPage - 1) * pageSize, currentPage * pageSize)
  ...
  const handleStatClick = useCallback((filter) => {
    ...
    } else if (filter === 'pendingCompany') {
      setActiveTab('company'); setStatusFilter('pending');
    } else if (filter === 'pendingGST') {
      setActiveTab('gst'); setStatusFilter('pending');
  ...
  const handleDownloadDocument = useCallback(async (doc) => {
    const safeName = `${doc.builderName || 'builder'}_${doc.label}`
      .replace(/[^a-z0-9]+/gi, '_')
      .replace(/^_+|_+$/g, '');
      ...
      setTimeout(() => {
        setBuilders(prev => {
          const updated = prev.map(builder => {
            if (builder.companyDoc.id === doc.id) {
              const companyDoc = { ...builder.companyDoc, status: newStatus };
              return recomputeOverall({ ...builder, companyDoc });
            }
            if (builder.gstDoc.id === doc.id) {
              const gstDoc = { ...builder.gstDoc, status: newStatus };
              return recomputeOverall({ ...builder, gstDoc });
            }
            if (builder.reraDoc.id === doc.id) {
              const reraDoc = { ...builder.reraDoc, status: newStatus };
              return recomputeOverall({ ...builder, reraDoc });
            }
            if (builder.companyDocs.some(d => d.id === doc.id)) {
              const companyDocs = builder.companyDocs.map(d => d.id === doc.id ? { ...d, status: newStatus } : d);
              return recomputeOverall({ ...builder, companyDocs });
            }
            return builder;
          });
          updateStats(updated);
        ...
        setViewingBuilder(prev => {
          if (!prev) return prev;
          if (prev.companyDoc.id === doc.id) return recomputeOverall({ ...prev, companyDoc: { ...prev.companyDoc, status: newStatus } });
          if (prev.gstDoc.id === doc.id) return recomputeOverall({ ...prev, gstDoc: { ...prev.gstDoc, status: newStatus } });
          if (prev.reraDoc.id === doc.id) return recomputeOverall({ ...prev, reraDoc: { ...prev.reraDoc, status: newStatus } });
          if (prev.companyDocs.some(d => d.id === doc.id)) {
            const companyDocs = prev.companyDocs.map(d => d.id === doc.id ? { ...d, status: newStatus } : d);
            return recomputeOverall({ ...prev, companyDocs });
          }
          return prev;
        });
  ...
  const recomputeOverall = (builder) => {
    const allStatuses = [builder.companyDoc.status, builder.gstDoc.status, builder.reraDoc.status, ...builder.companyDocs.map(d => d.status)];
    let overallStatus = 'pending';
    if (allStatuses.every(s => s === 'approved')) overallStatus = 'approved';
    else if (allStatuses.some(s => s === 'rejected')) overallStatus = 'rejected';
  ...
  const handleRejectDoc = useCallback((doc) => {
    setConfirmationModal({
      ...
      message: `Are you sure you want to reject "${doc.label}"? The builder will need to resubmit it.`,
  ...
  // ============ TAB CONFIG ============
  const tabs = [
    { key: 'all', label: 'All Builders', icon: <MdBusiness className="text-xs" /> },
    { key: 'company', label: 'Company Registration', icon: <FaBuilding className="text-xs" /> },
    { key: 'gst', label: 'GST Verification', icon: <FiCreditCard className="text-xs" /> },
    { key: 'rera', label: 'RERA Verification', icon: <FiAward className="text-xs" /> },
    { key: 'documents', label: 'Company Documents', icon: <FiFileText className="text-xs" /> },
  ];

  const statusOf = (builder) => {
    if (activeTab === 'company') return builder.companyDoc.status;
    if (activeTab === 'gst') return builder.gstDoc.status;
    if (activeTab === 'rera') return builder.reraDoc.status;
    if (activeTab === 'documents') {
      if (builder.companyDocs.some(d => d.status === 'rejected')) return 'rejected';
      if (builder.companyDocs.every(d => d.status === 'approved')) return 'approved';
      return 'pending';
    }
    return builder.overallStatus;
  };

  const getQuickDoc = (builder) => {
    if (activeTab === 'company') return builder.companyDoc;
    if (activeTab === 'gst') return builder.gstDoc;
    if (activeTab === 'rera') return builder.reraDoc;
    if (activeTab === 'documents') return builder.companyDocs.find(d => d.status === 'pending') || builder.companyDocs[0];
    return null;
  };
  ...
  return (
    <div className="space-y-6 p-4 lg:p-6 bg-[#F8FAF9] min-h-screen">
      ...
      <BuilderVerificationModal
        ...
        onClose={() => { setShowBuilderModal(false); setViewingBuilder(null); }}
        ...
        <div className="flex flex-col lg:flex-row items-start lg:items-center justify-between gap-4">
          <div>
            ...
            <p className="text-sm text-[#5A7D78] flex items-center gap-2 flex-wrap">
              <span>Company Registration · GST · RERA · Document approval in one place</span>
          ...
          <div className="bg-white rounded-2xl p-4 border border-[#E8F0EE] shadow-sm">
            <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-4 gap-3">
              <StatCard
                ...
                title="Total Builders" value={stats.total}
              ...
              <StatCard
                ...
                title="Pending Company" value={stats.pendingCompany}
                color="bg-gradient-to-br from-yellow-600 to-yellow-400"
                delay={100} isActive={activeFilter === 'pendingCompany'} onClick={() => handleStatClick('pendingCompany')}
              />
              <StatCard
                icon={<FiCreditCard className="text-white text-sm" />}
                title="Pending GST" value={stats.pendingGST}
                color="bg-gradient-to-br from-orange-600 to-orange-400"
                delay={150} isActive={activeFilter === 'pendingGST'} onClick={() => handleStatClick('pendingGST')}
              />
              <StatCard
                icon={<FiAward className="text-white text-sm" />}
                title="Pending RERA" value={stats.pendingRERA}
                color="bg-gradient-to-br from-blue-600 to-blue-400"
                delay={200} isActive={activeFilter === 'pendingRERA'} onClick={() => handleStatClick('pendingRERA')}
              />
              <StatCard
                icon={<FiFileText className="text-white text-sm" />}
                title="Pending Docs" value={stats.pendingDocs}
                color="bg-gradient-to-br from-purple-600 to-purple-400"
                delay={250} isActive={activeFilter === 'pendingDocs'} onClick={() => handleStatClick('pendingDocs')}
              />
              <StatCard
                icon={<MdVerified className="text-white text-sm" />}
                title="Fully Verified" value={stats.fullyVerified}
                color="bg-gradient-to-br from-emerald-600 to-emerald-400"
                delay={300} isActive={activeFilter === 'fullyVerified'} onClick={() => handleStatClick('fullyVerified')}
              />
              <StatCard
                ...
                delay={350} isActive={activeFilter === 'rejected'} onClick={() => handleStatClick('rejected')}
              />
            </div>
          </div>
        </div>
        ...
        <div className="flex flex-col lg:flex-row items-start lg:items-center gap-4">
          <div className="flex-1 w-full lg:w-auto relative">
            ...
            <input
              ref={searchInputRef}
              type="text"
              placeholder="Search builders by name, company, city..."
              value={searchQuery}
        ...
        {loading ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-4">
            {paginatedBuilders.map((builder, index) => {
              const badgeStatus = statusOf(builder);
              const quickDoc = getQuickDoc(builder);
              ...
              return (
                ...
                >
                    ...
                    <div className="flex items-center gap-3 min-w-0">
                      ...
                      <div className="min-w-0">
                        ...
                        <p className="text-[10px] text-[#5A7D78] truncate flex items-center gap-1">
                          <FaBuilding className="text-[9px]" /> {builder.companyName}
                        </p>
                      </div>
                    </div>
                    <button
                  ...
                  <div className="space-y-1.5 mb-3">
                    <button
                      onClick={() => handleOpenDocument(builder.companyDoc)}
                    ...
                    >
                      <span className="flex items-center gap-1.5 text-[10px] font-medium text-[#1A2E2A]">
                        <FaBuilding className="text-[#00695C] text-xs" /> Company
                      </span>
                      <StatusBadge status={builder.companyDoc.status} />
                    </button>
                    <button
                      onClick={() => handleOpenDocument(builder.gstDoc)}
                      className="w-full flex items-center justify-between px-2.5 py-1.5 bg-[#F5F9F8] rounded-lg hover:bg-[#E8F0EE] transition-all duration-300"
                    >
                      <span className="flex items-center gap-1.5 text-[10px] font-medium text-[#1A2E2A]">
                        <FiCreditCard className="text-[#00695C] text-xs" /> GST
                      </span>
                      <StatusBadge status={builder.gstDoc.status} />
                    </button>
                    <button
                      onClick={() => handleOpenDocument(builder.reraDoc)}
                      className="w-full flex items-center justify-between px-2.5 py-1.5 bg-[#F5F9F8] rounded-lg hover:bg-[#E8F0EE] transition-all duration-300"
                    >
                      <span className="flex items-center gap-1.5 text-[10px] font-medium text-[#1A2E2A]">
                        <FiAward className="text-[#00695C] text-xs" /> RERA
                      </span>
                      <StatusBadge status={builder.reraDoc.status} />
                    </button>
                      ...
                      {builder.companyDocs.map(doc => (
                        ...
                        >
                          <span className="flex items-center gap-1 text-[9px] font-medium text-[#1A2E2A] truncate">
                            <FiFileText className="text-[#00695C] text-[10px] shrink-0" /> {doc.label.split(' ')[0]}
                          </span>
                          <StatusBadge status={doc.status} />
                        </button>
                      ))}
          ...
          <div className="bg-white rounded-2xl border border-[#E8F0EE] shadow-sm overflow-hidden">
            ...
            {paginatedBuilders.map((builder) => {
              ...
              >
                ...
                <div className="col-span-2 text-xs text-[#5A7D78] truncate">{builder.companyName}</div>
                <div className="col-span-2 text-xs text-[#5A7D78] truncate">{builder.city}, {builder.state}</div>
                <div className="col-span-1 flex justify-center">
                  <button onClick={() => handleOpenDocument(builder.companyDoc)}><StatusBadge status={builder.companyDoc.status} /></button>
                </div>
                <div className="col-span-1 flex justify-center">
                  <button onClick={() => handleOpenDocument(builder.gstDoc)}><StatusBadge status={builder.gstDoc.status} /></button>
                </div>
                <div className="col-span-1 flex justify-center">
                  <button onClick={() => handleOpenDocument(builder.reraDoc)}><StatusBadge status={builder.reraDoc.status} /></button>
                </div>
        ...
        {paginatedBuilders.length === 0 && !loading && (
          <div className="flex flex-col items-center justify-center py-20 text-center bg-white rounded-2xl border border-[#E8F0EE]">
            <div className="w-24 h-24 rounded-full bg-[#F5F9F8] flex items-center justify-center mb-4 animate-float">
              <MdBusiness className="text-4xl text-[#B5C9C5]" />
            </div>
...
export default BuildersVerification;
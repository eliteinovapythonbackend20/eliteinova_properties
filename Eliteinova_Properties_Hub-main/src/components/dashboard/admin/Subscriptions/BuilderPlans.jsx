...
import { FaCity, FaCrown, FaGem, FaHome, FaHotel, FaRupeeSign, FaBuilding } from 'react-icons/fa';
...
// ============================================================
// DEFAULT BUILDER PLAN TYPES - Clean version (no limits/capabilities)
// ============================================================
const DEFAULT_PLAN_TYPES = {
  ...
  'Premium': {
    ...
    features: [
      'List up to 15 projects',
      'High search visibility',
      'Phone & email support',
      '180-day listing duration',
      '10 featured projects per month',
...
// ============================================================
const ViewBuilderDetailModal = ({ builder, show, onClose, onEdit, onDelete, planTypes }) => {
  if (!builder || !show) return null;

  const planConfig = planTypes[builder.subscriptionPlan] || planTypes['Basic'] || { theme: FALLBACK_THEME, label: builder.subscriptionPlan, icon: FiPackage };
      ...
      <div className="bg-white rounded-3xl max-w-4xl w-full max-h-[90vh] overflow-hidden shadow-2xl animate-slide-up border border-[#E8F0EE] flex flex-col">
        ...
        <div className="sticky bottom-0 px-6 py-4 bg-white border-t border-[#E8F0EE] rounded-b-3xl shrink-0 shadow-[0_-4px_20px_rgba(0,0,0,0.05)]">
          <div className="flex items-center gap-3">
            ...
            <button onClick={() => { if (onEdit) { onEdit(builder); onClose(); } }} className="flex-1 px-4 py-2.5 bg-[#26A69A] text-white rounded-xl hover:bg-[#1A8A7A] transition-all duration-300 text-sm font-medium shadow-lg shadow-[#26A69A]/30 hover:scale-[1.02]">
              <FiEdit className="inline mr-2" /> Edit
            </button>
            <button onClick={() => { if (onDelete) { onDelete(builder.id); } }} className="flex-1 px-4 py-2.5 bg-red-600 text-white rounded-xl hover:bg-red-700 transition-all duration-300 text-sm font-medium shadow-lg shadow-red-600/30 hover:scale-[1.02]">
...
// EDIT BUILDER MODAL
// ============================================================
const EditBuilderModal = ({ builder, show, onClose, onSave, planTypes }) => {
  if (!builder || !show) return null;
            ...
            <div className="bg-[#F5F9F8] rounded-2xl p-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                <div>
                  ...
                  <input type="text" name="builderId" value={formData.builderId} onChange={handleChange}
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
            ...
            <div className="bg-gradient-to-br from-purple-50 to-purple-100/50 rounded-2xl p-4 border border-purple-200">
              <div className="flex items-center gap-2 mb-2">
                ...
                <h4 className="text-xs font-bold text-purple-700 uppercase tracking-wider">Builders Using</h4>
              </div>
            ...
            <div className="bg-gradient-to-br from-amber-50 to-amber-100/50 rounded-2xl p-5 border border-amber-200">
              <div className="flex items-center gap-2 mb-3">
                ...
                <h4 className="text-sm font-bold text-amber-700 uppercase tracking-wider">
                  Builders on this Plan ({ownersOnPlan.length})
                </h4>
              </div>
              <div className="space-y-2 max-h-48 overflow-y-auto">
                {ownersOnPlan.map((builder) => (
                  <div key={builder.id} className="flex items-center gap-3 bg-white rounded-xl p-3 border border-amber-200 shadow-sm">
                    ...
                    <div className="min-w-0 flex-1">
                      <p className="text-sm font-bold text-[#1A2E2A] truncate">{builder.name}</p>
                      <p className="text-xs text-[#5A7D78] truncate">{builder.builderId} • {builder.email}</p>
                    </div>
      ...
      <div className="bg-white rounded-3xl max-w-4xl w-full max-h-[90vh] overflow-hidden shadow-2xl animate-slide-up border border-[#E8F0EE] flex flex-col">
        <div className="sticky top-0 p-6 rounded-t-3xl z-10 shrink-0" style={{ background: previewTheme.gradient }}>
          ...
          <h2 className="text-2xl font-bold text-white">
            {isNew ? 'Add New Builder Plan' : `Edit ${plan?.label || ''} Plan`}
          </h2>
          <p className="text-white/80 text-sm">
            {isNew ? 'Create a new subscription plan for builders' : 'Update plan details, pricing, and features'}
          </p>
        </div>
        ...
        <div className="bg-gradient-to-r from-[#E8F4F2] to-[#F5F9F8] rounded-xl p-3 mb-3 border border-[#E8F0EE]">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <span className="text-xs font-semibold text-[#5A7D78] uppercase tracking-wider">Builders Using</span>
            </div>
...
const BuilderPlans = () => {
  ...
  const generateMockBuilders = useCallback((planNames) => {
    ...
    for (let i = 1; i <= 60; i++) {
      ...
      list.push({
        id: `bld_${i}`, builderId: `BLD-${String(i).padStart(4, '0')}`, name,
  ...
  useEffect(() => {
    try {
      ...
      const mockBuilders = generateMockBuilders(planNames);
      setBuilders(mockBuilders);
      setFilteredBuilders(mockBuilders);
      computeStats(mockBuilders, DEFAULT_PLAN_TYPES);
    } catch (error) {
      console.error('Error generating mock builders:', error);
    }
  }, [generateMockBuilders, computeStats]);
  ...
  const handleDeletePlan = useCallback((planKey) => {
    ...
    if (!plan) return;
    const builderCount = builders.filter(b => b.subscriptionPlan === planKey).length;

    setConfirmationModal({
      isOpen: true,
      title: 'Delete Subscription Plan',
      message: `Are you sure you want to delete the "${plan.label}" plan?${builderCount > 0 ? ` ${builderCount} builder(s) are currently on this plan and will be moved to Basic plan.` : ''}`,
      ...
      onConfirm: () => {
        ...
        setTimeout(() => {
          ...
          setBuilders(prev => {
            ...
            const updated = prev.map(b => b.subscriptionPlan === planKey ? { ...b, subscriptionPlan: fallbackPlan } : b);
  ...
  }, [planTypes, builders, computeStats]);
  ...
  }, [builders, computeStats, planTypes]);
  ...
  const handleRefresh = useCallback(() => {
    ...
    setTimeout(() => {
      try {
        ...
        setBuilders(mockBuilders);
        setFilteredBuilders(mockBuilders);
        computeStats(mockBuilders, planTypes);
  ...
  }, [generateMockBuilders, planTypes, computeStats]);
  ...
  const handleExport = useCallback(() => {
    ...
    try {
      const data = filteredBuilders.map(b => ({
        'Builder ID': b.builderId || '', 'Name': b.name || '', 'Mail ID': b.email || '',
        'Phone Number': b.phone || '', 'Property Type': b.propertyType || '',
        'Subscription Plan': planTypes[b.subscriptionPlan]?.label || b.subscriptionPlan || '',
        'Street': b.street || '', 'Area / Locality': b.area || '', 'City': b.city || '',
        'District': b.district || '', 'State': b.state || '', 'Country': b.country || '', 'Pincode': b.pincode || ''
      }));
  ...
  }, [selectedBuilders, builders, computeStats, planTypes]);
  ...
  // Get builders for the viewing plan
  const buildersOnViewingPlan = viewingPlan
    ? builders.filter(b => b.subscriptionPlan === viewingPlan.name)
    : [];
  ...
  return (
    <div className="space-y-6 p-4 lg:p-6 bg-[#F8FAF9] min-h-screen">
      ...
      {showViewModal && viewingBuilder && (
        <ViewBuilderDetailModal builder={viewingBuilder} show={showViewModal}
          onClose={() => { setShowViewModal(false); setViewingBuilder(null); }}
          onEdit={handleEditBuilder} onDelete={handleDeleteBuilder} planTypes={planTypes} />
      )}

      {showEditModal && editingBuilder && (
        <EditBuilderModal builder={editingBuilder} show={showEditModal}
          onClose={() => { setShowEditModal(false); setEditingBuilder(null); }}
          onSave={handleSaveBuilder} planTypes={planTypes} />
      )}
      ...
      {showViewPlanModal && viewingPlan && (
        <ViewPlanModal plan={viewingPlan} show={showViewPlanModal}
          ...
          ownerCount={buildersOnViewingPlan.length}
          ownersOnPlan={buildersOnViewingPlan} />
      )}
        ...
        <div className="flex flex-col lg:flex-row items-start lg:items-center justify-between gap-4">
          <div>
            ...
            <p className="text-sm text-[#5A7D78] flex items-center gap-2 flex-wrap">
              <span>Manage subscription plans and property builders</span>
        ...
        <div className="relative animate-slide-in">
          <div className="bg-white rounded-2xl p-4 border border-[#E8F0EE] shadow-sm">
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
              <StatCard icon={<FiLayers className="text-white text-sm" />} title="Total Builders" value={stats.total}
                      ...
                      <div key={planKey}
                        ...
                        <div className="flex items-center gap-3 flex-wrap">
                          ...
                          <span className="text-xs font-bold text-purple-700 bg-purple-100 px-2 py-1 rounded-full flex items-center gap-1">
                            <FiUsers className="text-[10px]" /> {planStats[planKey] || 0} builders
                          </span>
      ...
      <div className="relative bg-white rounded-2xl p-4 shadow-sm border border-[#E8F0EE] hover:shadow-md transition-all duration-300">
        <div className="flex flex-col lg:flex-row items-start lg:items-center gap-4">
          <div className="flex-1 w-full relative">
            ...
            <input ref={searchInputRef} type="text" placeholder="Search builders by name, ID, email, phone, city..."
        ...
        {selectedBuilders.length > 0 && (
          <div className="mt-4 pt-4 border-t border-[#E8F0EE] flex flex-wrap items-center justify-between gap-3 animate-slide-in">
            ...
            <div className="flex flex-wrap items-center gap-2">
              ...
              <button onClick={() => setSelectedBuilders([])}
        ...
        {loading ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-4 gap-4">
            {paginatedBuilders.map((builder, index) => {
              ...
              const planConfig = planTypes[builder.subscriptionPlan] || { theme: FALLBACK_THEME, label: builder.subscriptionPlan, icon: FiPackage };
              const theme = planConfig.theme || FALLBACK_THEME;
              const PlanIcon = planConfig.icon || FiPackage;
              const propTypeConfig = PROPERTY_TYPE_CONFIG[builder.propertyType] || PROPERTY_TYPE_CONFIG['Individual'];
              const PropTypeIcon = propTypeConfig.icon;
                ...
                <div key={builder.id}
                  ...
                  <div className="flex items-start justify-between mb-2 gap-1">
                    <div className="flex items-center gap-2 min-w-0">
                      <input type="checkbox" checked={isSelected} onChange={() => handleSelectBuilder(builder.id)}
                  ...
                  <div className="flex flex-wrap gap-1 mt-2.5 pt-2.5 border-t border-[#E8F0EE]">
                    <button type="button" onClick={() => handleViewBuilder(builder)}
                    ...
                    <button type="button" onClick={() => handleEditBuilder(builder)}
                    ...
                    <button type="button" onClick={() => handleDeleteBuilder(builder.id)} disabled={actionLoading === builder.id}
          ...
          <div className="bg-white rounded-2xl border border-[#E8F0EE] shadow-sm overflow-hidden">
            <div className="grid grid-cols-12 gap-1 items-center px-4 py-3 bg-[#F5F9F8] border-b border-[#E8F0EE] text-xs font-semibold text-[#5A7D78] uppercase tracking-wider">
              <div className="col-span-2 flex items-center gap-2 min-w-0">
                <input type="checkbox" checked={selectedBuilders.length === paginatedBuilders.length && paginatedBuilders.length > 0}
            ...
            {paginatedBuilders.map((builder, index) => {
              ...
              return (
                <div key={builder.id}
                  ...
                  <div className="col-span-2 min-w-0 text-xs font-medium text-[#5A7D78] truncate">{builder.email}</div>
                  <div className="col-span-1 min-w-0 text-xs font-medium text-[#5A7D78] truncate">{builder.phone}</div>
                  ...
                  <div className="col-span-1 min-w-0 text-xs font-medium text-[#5A7D78] truncate">{builder.city}</div>
                  <div className="col-span-1 min-w-0 text-xs font-medium text-[#5A7D78] truncate">{builder.state}</div>
        ...
        <div className="flex flex-wrap items-center justify-between bg-white rounded-2xl px-4 py-3 border border-[#E8F0EE] shadow-sm gap-3">
          <div className="flex items-center gap-2 text-sm text-[#5A7D78] flex-wrap">
            <span className="font-medium">
              Showing {(currentPage - 1) * pageSize + 1} to {Math.min(currentPage * pageSize, filteredBuilders.length)} of {filteredBuilders.length} builders
            </span>

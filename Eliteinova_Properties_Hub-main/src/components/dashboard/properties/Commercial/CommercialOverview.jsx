// src/components/dashboard/properties/Commercial/CommercialOverview.jsx
...
import {
  ...
  FiMap, FiActivity, FiLayers, FiShoppingBag, FiLayout, FiUsers, FiCpu, FiBox, FiTruck
} from 'react-icons/fi';
import {
  FaBuilding, FaCity, FaWarehouse, FaIndustry, FaStore, FaUtensils, FaHotel,
  FaHospital, FaGraduationCap, FaFilm, FaGasPump, FaTractor
} from 'react-icons/fa';
...
// ============================================================
// `filterValue` is the exact property_type string the real posting forms
// (ComSellForm/ComRentForm/ComLeaseForm, and their Builder/PropertyManagement
// equivalents) submit to the backend - only 12 real values exist across all
// of them: Retail Shop, Office Space, Showroom, Warehouse, Commercial
// Complex, Shopping Mall, Restaurant, Hotel, Co-working Space, Educational
// Institution, Clinic, Petrol Bunk. `label` is left untouched (still what's
// shown in the UI) even where it differs from filterValue. The subtypes
// with no real posting-form equivalent (Commercial Land Plot, Industrial
// Property, Business Center, IT Park/Tech Park, Multiplex/Entertainment,
// Cold Storage/Logistics Hub, Mixed-use Commercial, Agricultural Commercial)
// keep a filterValue that matches no real listing - kept as-is, same
// treatment as Apartment's Rental/Lease/Studio pages.
const PROPERTY_TYPES = {
  ...
  'CommercialLand': {
    ...
    label: 'Commercial Land / Plot',
    filterValue: 'Commercial Land Plot'
  },
  'WarehouseGodown': {
    icon: FaWarehouse,
  ...
  'IndustrialFactory': {
    ...
    label: 'Industrial Property / Factory',
    filterValue: 'Industrial Property'
  },
  'CoworkingSpace': {
    icon: FiUsers,
  ...
  'CommercialComplex': {
    ...
    filterValue: 'Commercial Complex'
  },
  'RestaurantCafeSpace': {
    icon: FaUtensils,
  ...
  'HotelLodgeResort': {
    ...
    label: 'Hotel / Lodge / Resort Property',
    filterValue: 'Hotel'
  },
  'ClinicHospitalSpace': {
    icon: FaHospital,
  ...
  'EducationalInstitution': {
    ...
    label: 'Educational Institution Property',
    filterValue: 'Educational Institution'
  },
  'ITTechParkSpace': {
    icon: FiCpu,
    ...
    label: 'IT Park / Tech Park Space',
    filterValue: 'IT Park / Tech Park Space'
  },
  'MultiplexEntertainment': {
    icon: FaFilm,
    color: 'from-zinc-600 to-zinc-400',
    bg: 'bg-zinc-50',
    text: 'text-zinc-700',
    border: 'border-zinc-200',
    label: 'Multiplex / Entertainment Space',
    filterValue: 'Multiplex / Entertainment Space'
  },
  'PetrolBunkFuelStation': {
    icon: FaGasPump,
    color: 'from-yellow-600 to-yellow-400',
    bg: 'bg-yellow-50',
    text: 'text-yellow-700',
    border: 'border-yellow-200',
    label: 'Petrol Bunk / Fuel Station',
    filterValue: 'Petrol Bunk'
  },
  'ColdStorageLogisticsHub': {
    icon: FiTruck,
    color: 'from-slate-600 to-slate-400',
    bg: 'bg-slate-50',
    text: 'text-slate-700',
    border: 'border-slate-200',
    label: 'Cold Storage / Logistics Hub',
    filterValue: 'Cold Storage / Logistics Hub'
  },
  'MixedUseCommercial': {
    icon: FiBox,
    ...
    label: 'Mixed-use Commercial Property',
    filterValue: 'Mixed-use Commercial Property'
  },
  'AgriculturalCommercial': {
    ...
    label: 'Agricultural Commercial Property',
    filterValue: 'Agricultural Commercial Property'
  }
};

// Reverse lookup: real backend property_type value -> the PascalCase key
// every render path below already indexes PROPERTY_TYPES/RESTRICTED_LISTING_TYPES by.
...
// Every commercial subcategory allows Buy, Rent or Lease.
...
const ViewPropertyDetailModal = ({ property, show, onClose, onEdit, onDelete }) => {
  ...
  const typeConfig = PROPERTY_TYPES[property.propertyType] || PROPERTY_TYPES['OfficeSpace'];
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm animate-fade-in">
      <div className="bg-white rounded-3xl max-w-4xl w-full max-h-[90vh] overflow-hidden shadow-2xl animate-slide-up border border-[#E8F0EE] flex flex-col">
        ...
        <div className="sticky top-0 bg-gradient-to-r from-[#00695C] to-[#26A69A] p-6 rounded-t-3xl z-10 shrink-0">
          ...
          <h2 className="text-2xl font-bold text-white">Edit Commercial Property</h2>
          <p className="text-white/80 text-sm">Update commercial property details</p>
        </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div className="bg-[#F5F9F8] rounded-2xl p-4">
              <h3 className="text-xs font-semibold text-[#5A7D78] uppercase tracking-wider mb-3 flex items-center gap-2">
                <FiBriefcase className="text-[#00695C]" />
                Basic Details
              </h3>
...
const CommercialOverview = () => {
  ...
  const mapCard = useCallback((card) => ({
    ...
    propertyType: propertyTypeKeyFromValue(card.propertyType) || card.propertyType || '',
  ...
  const {
    ...
    clearSearch,
    activePropertyType, setActivePropertyType,
  ...
  // rawStats.byType is keyed by the real property_type strings - translate
  // each PascalCase config key through its filterValue to read the matching
  // count (the fake-typed subtypes' filterValue will always read as 0,
  // consistent with those pages never matching a real listing).
  const stats = { total: rawStats.total || 0 };
  Object.keys(PROPERTY_TYPES).forEach((key) => {
    stats[key] = rawStats.byType?.[PROPERTY_TYPES[key].filterValue] || 0;
  });
  ...
  // wire - propertyType is translated from its internal PascalCase config
  // key back to the real filterValue the posting forms use (subtypes with
  // no real posting-form equivalent never match a real listing, by design -
  // see the PROPERTY_TYPES comment above).
  const handleSaveProperty = useCallback((updatedProperty) => {
    adminDashboardService.updateProperty(updatedProperty.id, {
      ...
      propertyType: PROPERTY_TYPES[updatedProperty.propertyType]?.filterValue,
  ...
  // ============ STAT CLICK HANDLERS ============
  const handleTypeClick = useCallback((filterValue) => {
    setActivePropertyType(activePropertyType === filterValue ? 'all' : filterValue);
  ...
  const handleExport = useCallback(async () => {
    ...
    try {
      ...
      a.href = url;
      a.download = `commercial_properties_${new Date().toISOString().split('T')[0]}.csv`;
      a.click();
        ...
        <div className="flex flex-col lg:flex-row items-start lg:items-center justify-between gap-4">
          <div>
            ...
            <p className="text-sm text-[#5A7D78] flex items-center gap-2 flex-wrap">
              <span>Office, Retail, Industrial, Hospitality, Institutional &amp; other commercial listings</span>
              ...
              {Object.keys(PROPERTY_TYPES).map((type, idx) => {
                ...
                return (
                  <StatCard
                    ...
                    delay={(idx + 1) * 40}
                    isActive={activePropertyType === config.filterValue}
                    onClick={() => handleTypeClick(config.filterValue)}
                  />
                );
              })}
            </div>
        ...
        {loading ? (
          ...
          <div className="bg-white rounded-2xl border border-[#E8F0EE] shadow-sm overflow-hidden">
            <div className="grid grid-cols-12 gap-1 items-center px-4 py-3 bg-[#F5F9F8] border-b border-[#E8F0EE] text-xs font-semibold text-[#5A7D78] uppercase tracking-wider">
              ...
              {/* ===== Reordered: City before State to follow Location→Area→City→District→State→Pincode hierarchy ===== */}
        ...
        {paginatedProperties.length === 0 && !loading && (
          <div className="flex flex-col items-center justify-center py-20 text-center bg-white rounded-2xl border border-[#E8F0EE]">
            <div className="w-24 h-24 rounded-full bg-[#F5F9F8] flex items-center justify-center mb-4 animate-float">
              <FiBriefcase className="text-4xl text-[#B5C9C5]" />
            </div>
            <h3 className="text-xl font-bold text-[#1A2E2A]">No properties found</h3>
            <p className="text-sm text-[#5A7D78] mt-1">
              {filterCount > 0 ? 'Try adjusting your search or filter criteria' : 'No commercial properties have been added yet'}
            </p>
            {filterCount > 0 && (
              <button
                onClick={clearAllFilters}

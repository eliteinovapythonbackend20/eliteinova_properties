// src/components/dashboard/properties/Individual/IndividualOverview.jsx
...
const ViewPropertyDetailModal = ({ property, show, onClose, onEdit, onDelete }) => {
  ...
  const typeConfig = PROPERTY_TYPES[property.propertyType] || PROPERTY_TYPES['Independent House'];
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm animate-fade-in">
      <div className="bg-white rounded-3xl max-w-4xl w-full max-h-[90vh] overflow-hidden shadow-2xl animate-slide-up border border-[#E8F0EE] flex flex-col">
        ...
        <div className="sticky top-0 bg-gradient-to-r from-[#00695C] to-[#26A69A] p-6 rounded-t-3xl z-10 shrink-0">
          ...
          <p className="text-white/80 text-sm">Update individual property details</p>
        </div>
...
const IndividualOverview = () => {
  ...
  // rawStats.byType is keyed by the real property_type strings (see
  // PROPERTY_TYPES above, which already match the posting forms exactly).
  const stats = {
    total: rawStats.total || 0,
    independentHouse: rawStats.byType?.['Independent House'] || 0,
    independentVilla: rawStats.byType?.['Independent Villa'] || 0,
    duplexResidentialUnit: rawStats.byType?.['Duplex Residential Unit'] || 0,
  };
  ...
  // wire - propertyType is safe to include here (unlike the per-subtype
  // pages, where it's fixed) since PROPERTY_TYPES above already matches the
  // real posting-form values within this one category.
  ...
  const handleTypeClick = useCallback((type) => {
    setActivePropertyType(activePropertyType === type ? 'all' : type);
  ...
  const handleExport = useCallback(async () => {
    ...
    try {
      ...
      a.href = url;
      a.download = `individual_properties_${new Date().toISOString().split('T')[0]}.csv`;
      a.click();
        ...
        <div className="flex flex-col lg:flex-row items-start lg:items-center justify-between gap-4">
          <div>
            ...
            <p className="text-sm text-[#5A7D78] flex items-center gap-2 flex-wrap">
              <span>Independent House, Independent Villa &amp; Duplex Residential Unit listings</span>
          ...
          <div className="bg-white rounded-2xl p-4 border border-[#E8F0EE] shadow-sm">
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
              ...
              <StatCard
                ...
                delay={100}
                isActive={activePropertyType === 'Independent House'}
                onClick={() => handleTypeClick('Independent House')}
              />
              <StatCard
                icon={<FiHome className="text-white text-sm" />}
                title="Independent Villa"
                value={stats.independentVilla}
                color="bg-gradient-to-br from-purple-600 to-purple-400"
                delay={200}
                isActive={activePropertyType === 'Independent Villa'}
                onClick={() => handleTypeClick('Independent Villa')}
              />
              <StatCard
                icon={<FaBuilding className="text-white text-sm" />}
                title="Duplex Residential Unit"
                value={stats.duplexResidentialUnit}
                color="bg-gradient-to-br from-emerald-600 to-emerald-400"
                delay={300}
                isActive={activePropertyType === 'Duplex Residential Unit'}
                onClick={() => handleTypeClick('Duplex Residential Unit')}
              />
            </div>
          </div>
        </div>
        ...
        {paginatedProperties.length === 0 && !loading && (
          <div className="flex flex-col items-center justify-center py-20 text-center bg-white rounded-2xl border border-[#E8F0EE]">
            ...
            <p className="text-sm text-[#5A7D78] mt-1">
              {filterCount > 0 ? 'Try adjusting your search or filter criteria' : 'No individual properties have been added yet'}
            </p>
            {filterCount > 0 && (
              <button
                onClick={clearAllFilters}

// src/components/dashboard/properties/Hostel/GirlsHostel.jsx
...
import {
  ...
  FiChevronUp, FiCheckCircle, FiXCircle, FiHome, FiGlobe,
  FiMap, FiActivity, FiDollarSign, FiKey
} from 'react-icons/fi';
import { FaCity, FaFemale } from 'react-icons/fa';

// ============================================================
// FIXED PROPERTY TYPE — this page only handles Girls Hostel
// ============================================================
const GIRLS_HOSTEL_TYPE = {
  icon: FaFemale,
      ...
      <div className="bg-white rounded-3xl max-w-4xl w-full max-h-[90vh] overflow-hidden shadow-2xl animate-slide-up border border-[#E8F0EE] flex flex-col">
        ...
        <div className="sticky top-0 bg-gradient-to-r from-[#00695C] to-[#26A69A] p-6 rounded-t-3xl z-10 shrink-0">
          ...
          <div className="flex items-center gap-2 flex-wrap">
            <span className={`px-3 py-1 rounded-full text-xs font-semibold flex items-center gap-1.5 ${GIRLS_HOSTEL_TYPE.bg} ${GIRLS_HOSTEL_TYPE.text} border ${GIRLS_HOSTEL_TYPE.border}`}>
              <FaFemale className="text-xs" /> {GIRLS_HOSTEL_TYPE.label}
            </span>
          </div>
        </div>
        ...
        <div className="flex-1 overflow-y-auto p-6 bg-white">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div className="bg-[#F5F9F8] rounded-2xl p-4">
              ...
              <p className="text-sm font-bold text-[#1A2E2A]">{GIRLS_HOSTEL_TYPE.label}</p>
            </div>

            <div className="bg-[#F5F9F8] rounded-2xl p-4">
              <div className="flex items-center gap-2 mb-1">
                <FaFemale className="text-[#00695C] text-sm" />
...
const EditPropertyModal = ({ property, show, onClose, onSave }) => {
  ...
  const handleSubmit = (e) => {
    ...
    setLoading(true);
    setTimeout(() => {
      onSave({ ...property, ...formData, propertyType: GIRLS_HOSTEL_TYPE.label });
      setLoading(false);
      onClose();
    }, 700);
  };
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm animate-fade-in">
      <div className="bg-white rounded-3xl max-w-4xl w-full max-h-[90vh] overflow-hidden shadow-2xl animate-slide-up border border-[#E8F0EE] flex flex-col">
        ...
        <div className="sticky top-0 bg-gradient-to-r from-[#00695C] to-[#26A69A] p-6 rounded-t-3xl z-10 shrink-0">
          ...
          <h2 className="text-2xl font-bold text-white">Edit Girls Hostel</h2>
          <p className="text-white/80 text-sm">Update girls hostel property details</p>
        </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div className="bg-[#F5F9F8] rounded-2xl p-4">
              <h3 className="text-xs font-semibold text-[#5A7D78] uppercase tracking-wider mb-3 flex items-center gap-2">
                <FaFemale className="text-[#00695C]" />
                Basic Details
              </h3>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                ...
                <div>
                  ...
                  <input
                    type="text" value={GIRLS_HOSTEL_TYPE.label} disabled
...
const GirlsHostel = () => {
  ...
  const normalizeProperties = useCallback((list) => {
    ...
    return list.map(p => ({
      ...p,
      propertyType: GIRLS_HOSTEL_TYPE.label
    }));
  }, []);
  ...
  const generateMockProperties = useCallback(() => {
    const propertyTitles = [
      'Sunrise Girls Hostel', 'Silver Oak Girls Hostel', 'Rose Garden Girls Hostel',
      'Lotus Girls Hostel', 'Orchid Girls Hostel', 'Maple Leaf Girls Hostel',
      'Golden Petal Girls Hostel', 'Emerald Girls Hostel', 'Riverside Girls Hostel',
      'Jasmine Girls Hostel', 'Willow Girls Hostel', 'Crimson Girls Hostel',
      'Serene Girls Hostel', 'Blossom Girls Hostel', 'Ivy Girls Hostel',
      'Meadow Girls Hostel', 'Starlight Girls Hostel', 'Cherry Blossom Girls Hostel'
    ];
    ...
    for (let i = 1; i <= 40; i++) {
      ...
      propertiesList.push({
        id: `gh_${i}`,
        propertyId: `GH-${String(i).padStart(4, '0')}`,
        propertyTitle: propertyTitles[Math.floor(Math.random() * propertyTitles.length)],
        propertyType: GIRLS_HOSTEL_TYPE.label,
        listingType,
        description: `A well-maintained girls hostel with secure premises, good amenities and prime connectivity, available for ${listingType.toLowerCase()}.`,
  ...
  const handleExport = useCallback(() => {
    ...
    try {
      ...
      a.href = url;
      a.download = `girls_hostel_${new Date().toISOString().split('T')[0]}.csv`;
      a.click();
        ...
        <div className="flex flex-col lg:flex-row items-start lg:items-center justify-between gap-4">
          <div>
            ...
            <p className="text-sm text-[#5A7D78] flex items-center gap-2 flex-wrap">
              <span>All Girls Hostel listings across Buy, Rent &amp; Lease</span>
        ...
        <div className="relative animate-slide-in">
          <div className="bg-white rounded-2xl p-4 border border-[#E8F0EE] shadow-sm">
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
              <StatCard
                icon={<FaFemale className="text-white text-sm" />}
                ...
                >
                  ...
                  <div className="space-y-1">
                    <div className="flex items-center gap-2 text-[11px] text-[#5A7D78]">
                      <FaFemale className="text-[#00695C] flex-shrink-0" />
                      <span className="truncate font-semibold text-[#1A2E2A]">{GIRLS_HOSTEL_TYPE.label}</span>
                    </div>
                    <div className="flex items-center gap-2 text-[11px] text-[#5A7D78]">
        ...
        {paginatedProperties.length === 0 && !loading && (
          <div className="flex flex-col items-center justify-center py-20 text-center bg-white rounded-2xl border border-[#E8F0EE]">
            <div className="w-24 h-24 rounded-full bg-[#F5F9F8] flex items-center justify-center mb-4 animate-float">
              <FaFemale className="text-4xl text-[#B5C9C5]" />
            </div>
            <h3 className="text-xl font-bold text-[#1A2E2A]">No properties found</h3>
            <p className="text-sm text-[#5A7D78] mt-1">
              {filterCount > 0 ? 'Try adjusting your search or filter criteria' : 'No girls hostel listings have been added yet'}
            </p>
            {filterCount > 0 && (
              <button
                onClick={clearAllFilters}

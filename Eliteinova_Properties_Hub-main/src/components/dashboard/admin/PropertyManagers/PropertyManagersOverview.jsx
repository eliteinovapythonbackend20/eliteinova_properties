// src/components/dashboard/admin/PropertyManagers/PropertyManagersOverview.jsx
...
import {
  ...
  FiTool, FiFilePlus, FiStar as FiStarIcon
} from 'react-icons/fi';
import {
  ...
  FaBuilding as FaBuildingSolid, FaUsersCog, FaToolbox, FaClipboardCheck
} from 'react-icons/fa';
import {
  MdOutlineRealEstateAgent, MdApartment, MdVerified, MdOutlineVerified,
  MdAssignment, MdAttachMoney, MdBusinessCenter, MdManageAccounts,
  MdOutlineBusiness, MdOutlineBuild, MdOutlineHomeWork, MdOutlineMiscellaneousServices,
  MdOutlineFilePresent, MdOutlineVerifiedUser, MdOutlinePeopleAlt
} from 'react-icons/md';
...
// MAIN COMPONENT
// ============================================================
const PropertyManagersOverview = () => {
  ...
  const [stats, setStats] = useState({
    totalCompanies: 28,
    registrationApproval: 12,
    companyManagement: 8,
    maintenance: 45,
    propertyControl: 32,
    pendingApprovals: 12,
    verifiedCompanies: 16,
    unverifiedCompanies: 6,
    totalProperties: 187,
    pendingListings: 32,
    featuredProperties: 23,
    verifiedProperties: 89,
    totalStaff: 142,
    totalOwners: 65,
    totalTenants: 98,
    totalComplaints: 27,
    totalRentCollected: 2450000,
    growthRate: 12.5,
    monthlyGrowth: 7.8,
    newThisWeek: 4,
    activeListings: 155,
    totalLeases: 76,
    pendingServiceRequests: 18,
    leaseRenewals: 12
  });
  ...
  const quickStats = useMemo(() => [
    { icon: <FaBuilding className="text-[#00695C]" />, label: 'Companies', value: stats.totalCompanies },
    { icon: <FiUserPlus className="text-blue-500" />, label: 'New This Week', value: stats.newThisWeek },
    { icon: <FiCheckCircle className="text-emerald-500" />, label: 'Verified', value: stats.verifiedCompanies },
  ...
  const handleRefresh = useCallback(() => {
    ...
    setTimeout(() => {
      setStats(prev => ({
        ...
        registrationApproval: Math.max(0, prev.registrationApproval + Math.floor(Math.random() * 4) - 2),
        companyManagement: prev.companyManagement + Math.floor(Math.random() * 3),
        maintenance: prev.maintenance + Math.floor(Math.random() * 5),
        propertyControl: Math.max(0, prev.propertyControl + Math.floor(Math.random() * 4) - 2),
        newThisWeek: prev.newThisWeek + Math.floor(Math.random() * 2),
        totalComplaints: Math.max(0, prev.totalComplaints + Math.floor(Math.random() * 4) - 2),
      }));
      setLoading(false);
    ...
    <div className="space-y-6 p-4 lg:p-6 bg-gradient-to-br from-[#f8fafc] via-white to-[#f1f5f9] min-h-screen relative">
        ...
        <div className="flex flex-col gap-4">
          <div className="flex items-center gap-3 flex-wrap">
            <div className="p-2.5 bg-gradient-to-br from-[#00695C] to-[#26A69A] rounded-2xl shadow-lg animate-pulse-glow relative">
              ...
              <MdBusinessCenter className="text-white text-xl relative z-10" />
            </div>
            <div>
              ...
              <div className="flex items-center gap-2 text-sm text-gray-600 flex-wrap">
                <span>Complete control over property management companies</span>
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-3 gap-4 relative">
        <StatCard
          ...
          onClick={() => navigateTo('/admin/property-managers/overview')}
          trend="up"
          trendValue="12.5"
        />
        <StatCard
          ...
          subtitle="Pending KYC & verification"
          color="bg-gradient-to-br"
          gradient="from-amber-500 to-amber-400"
          borderColor="border-amber-500/30"
          delay={100}
          statsAnimating={statsAnimating}
          onClick={() => navigateTo('/admin/property-managers/registration')}
          trend="down"
          trendValue="4.8"
        />
        <StatCard
          ...
          value={stats.companyManagement}
          subtitle={`${stats.verifiedCompanies} verified`}
          color="bg-gradient-to-br"
          gradient="from-purple-600 to-purple-400"
          borderColor="border-purple-600/30"
          delay={200}
          statsAnimating={statsAnimating}
          onClick={() => navigateTo('/admin/property-managers/companies')}
          trend="up"
          trendValue="18.2"
        />
        <StatCard
          icon={<FiTool className="text-lg text-white" />}
          title="Maintenance"
          value={stats.maintenance}
          subtitle={`${stats.totalComplaints} complaints`}
          color="bg-gradient-to-br"
          gradient="from-rose-600 to-rose-400"
          borderColor="border-rose-600/30"
          delay={300}
          statsAnimating={statsAnimating}
          onClick={() => navigateTo('/admin/property-managers/maintenance')}
          trend="up"
          trendValue="9.6"
        />
        <StatCard
          ...
          delay={400}
          statsAnimating={statsAnimating}
          onClick={() => navigateTo('/admin/property-managers/property-control')}
          trend="up"
          trendValue="14.3"
        />
      </div>
      ...
      <div>
        <SectionHeader
          ...
          subtitle="Manage companies, properties, and maintenance"
        />
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
          ...
          <NavCard
            ...
            title="Registration Approval"
            description="Approve/reject owners, KYC verification, mobile, email, Aadhaar & PAN verification."
            color="bg-gradient-to-br"
            gradient="from-[#00695C] to-[#26A69A]"
            borderColor="border-[#00695C]/30"
            delay={0}
            ...
            stats={[
              ...
              { label: 'Verified', value: stats.verifiedCompanies },
          ...
          <NavCard
            icon={<MdManageAccounts className="text-white text-xl" />}
            title="Company Management"
            description="Company registration, staff, owner, tenant management & property assignment."
            color="bg-gradient-to-br"
            gradient="from-purple-600 to-purple-400"
            borderColor="border-purple-600/30"
            delay={100}
            onClick={() => navigateTo('/admin/property-managers/companies')}
            badge={`${stats.totalStaff} Staff`}
          ...
          <NavCard
            ...
            title="Maintenance"
            description="Complaint management, rent collection, lease renewal & service requests."
            color="bg-gradient-to-br"
            gradient="from-rose-600 to-rose-400"
            borderColor="border-rose-600/30"
            delay={200}
            onClick={() => navigateTo('/admin/property-managers/maintenance')}
            badge={`${stats.pendingServiceRequests} Requests`}
            badgeColor="bg-gradient-to-r from-rose-500 to-rose-400"
            stats={[
              { label: 'Complaints', value: stats.totalComplaints },
              { label: 'Rent', value: '₹24.5L' },
              { label: 'Leases', value: stats.totalLeases },
            ]}
          />
      ...
      {/* <div>
        <SectionHeader
          ...
          subtitle="Live updates from property management companies"
          action={true}
          actionLabel="View All"
      ...
      <div className="text-center pt-4">
        <p className="text-[11px] text-gray-500 flex items-center justify-center gap-2">
          ...
          <span>© 2026 Property Management Companies • All rights reserved</span>
          <span className="w-4 h-0.5 bg-gray-300 rounded-full" />
        </p>
      </div>

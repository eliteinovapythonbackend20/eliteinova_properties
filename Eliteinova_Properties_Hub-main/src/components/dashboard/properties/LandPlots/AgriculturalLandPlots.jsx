// src/components/dashboard/properties/LandPlots/AgriculturalLandPlots.jsx
...
import { FaCity, FaTractor, FaSeedling, FaLeaf, FaTree, FaAppleAlt, FaCoffee, FaDove, FaTint, FaFish } from 'react-icons/fa';

// ============================================================
// FIXED MAIN PROPERTY TYPE — this page only handles Agricultural Land / Plots
// ============================================================
const MAIN_TYPE_LABEL = 'Agricultural Land / Plots';
...
const SUB_TYPES = {
  ...
  'FisheriesAquacultureLand': {
    ...
    label: 'Fisheries / Aquaculture Land'
  }
};
...
const ViewPropertyDetailModal = ({ property, show, onClose, onEdit, onDelete }) => {
  ...
  const subConfig = SUB_TYPES[property.propertySubType] || SUB_TYPES['AgriculturalLand'];
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm animate-fade-in">
      <div className="bg-white rounded-3xl max-w-4xl w-full max-h-[90vh] overflow-hidden shadow-2xl animate-slide-up border border-[#E8F0EE] flex flex-col">
        ...
        <div className="sticky top-0 bg-gradient-to-r from-[#00695C] to-[#26A69A] p-6 rounded-t-3xl z-10 shrink-0">
          ...
          <h2 className="text-2xl font-bold text-white">Edit Agricultural Land / Plot</h2>
          <p className="text-white/80 text-sm">Update agricultural land / plot property details</p>
        </div>
...
const AgriculturalLandPlots = () => {
  ...
  const generateMockProperties = useCallback(() => {
    const propertyTitles = [
      'Green Valley Agricultural Land', 'Golden Harvest Farm Land', 'Nature Fresh Organic Farming Land',
      'Coastal Coconut Farm Land', 'Orchard Hill Mango Grove Land', 'Highland Tea / Coffee Estate',
      'Sunrise Poultry Farm Land', 'Meadow Dairy Farm Land', 'Blue Lagoon Fisheries Land',
      'Riverside Agricultural Land', 'Silverline Farm Land', 'Union Organic Farming Land',
      'Crown Coconut Farm Land', 'Emerald Mango Grove Land', 'Fortune Tea / Coffee Estate',
      'Palm Grove Poultry Farm Land', 'Meridian Dairy Farm Land', 'Junction Fisheries / Aquaculture Land'
    ];
    ...
    for (let i = 1; i <= 60; i++) {
      ...
      propertiesList.push({
        id: `al_${i}`,
        propertyId: `AL-${String(i).padStart(4, '0')}`,
        propertyTitle: propertyTitles[Math.floor(Math.random() * propertyTitles.length)],
        propertyType: MAIN_TYPE_LABEL,
        propertySubType,
        listingType,
        description: `A well-located ${SUB_TYPES[propertySubType].label.toLowerCase()} under agricultural land / plots, available for ${listingType.toLowerCase()}.`,
  ...
  const handleExport = useCallback(() => {
    ...
    try {
      ...
      a.href = url;
      a.download = `agricultural_land_plots_${new Date().toISOString().split('T')[0]}.csv`;
      a.click();
        ...
        <div className="flex flex-col lg:flex-row items-start lg:items-center justify-between gap-4">
          <div>
            ...
            <p className="text-sm text-[#5A7D78] flex items-center gap-2 flex-wrap">
              <span>All Agricultural Land / Plots sub types across Buy, Rent &amp; Lease</span>
        ...
        {paginatedProperties.length === 0 && !loading && (
          <div className="flex flex-col items-center justify-center py-20 text-center bg-white rounded-2xl border border-[#E8F0EE]">
            ...
            <p className="text-sm text-[#5A7D78] mt-1">
              {filterCount > 0 ? 'Try adjusting your search or filter criteria' : 'No agricultural land / plot listings have been added yet'}
            </p>
            {filterCount > 0 && (
              <button
                onClick={clearAllFilters}
...
export default AgriculturalLandPlots;
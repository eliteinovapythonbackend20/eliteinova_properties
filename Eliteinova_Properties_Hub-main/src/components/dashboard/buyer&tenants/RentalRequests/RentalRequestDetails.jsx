// src/components/dashboard/admin/buyer&tenants/RentalRequests/RentalRequestDetails.jsx
...
import {
  ...
  FaBriefcase as FaBriefcaseSolid, FaComments
} from 'react-icons/fa';
import { MdOutlineVerified, MdOutlineFamilyRestroom, MdOutlineSecurity } from 'react-icons/md';
...
/* ============================================================
   VIEW RENTAL REQUEST DETAILS MODAL
============================================================ */

const ViewRentalRequestModal = ({ request, show, onClose, onEdit, onDelete }) => {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm animate-fade-in">
      <div className="bg-white rounded-3xl max-w-2xl w-full max-h-[90vh] overflow-hidden shadow-2xl animate-slide-up border border-[#E8F0EE] flex flex-col">
        ...
        <div className="sticky top-0 bg-gradient-to-r from-[#00695C] to-[#26A69A] p-6 rounded-t-3xl z-10 shrink-0">
          ...
          <h2 className="text-2xl font-bold text-white">Rental Request Details</h2>
        ...
        <div className="flex-1 overflow-y-auto p-6 bg-white">
          <div className="space-y-6">
            ...
            <div className="bg-[#F5F9F8] rounded-2xl p-4">
              ...
              <div className="flex items-start gap-4">
                <div className="w-14 h-14 rounded-2xl bg-gradient-to-br from-[#00695C] to-[#26A69A] flex items-center justify-center text-white font-bold text-xl shadow-lg shrink-0">
                  {request.tenantName.split(' ').map(w => w[0]).join('').slice(0, 2).toUpperCase()}
                </div>
                <div className="flex-1 min-w-0">
                  <p className="text-lg font-bold text-[#1A2E2A]">{request.tenantName}</p>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="bg-[#F5F9F8] rounded-2xl p-4">
                <div className="flex items-center gap-2 mb-1">
                  ...
                  <h4 className="text-xs font-semibold text-[#5A7D78] uppercase tracking-wider">Monthly Rent</h4>
                </div>
                <p className="text-2xl font-bold text-[#1A2E2A]">₹{request.monthlyRent.toLocaleString()}</p>
              </div>

              <div className="bg-[#F5F9F8] rounded-2xl p-4">
                <div className="flex items-center gap-2 mb-1">
                  <MdOutlineSecurity className="text-[#00695C] text-sm" />
                  <h4 className="text-xs font-semibold text-[#5A7D78] uppercase tracking-wider">Security Deposit</h4>
                </div>
                <p className="text-2xl font-bold text-[#1A2E2A]">₹{request.securityDeposit.toLocaleString()}</p>
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div className="bg-[#F5F9F8] rounded-2xl p-4">
                <div className="flex items-center gap-2 mb-1">
                  ...
                  <h4 className="text-xs font-semibold text-[#5A7D78] uppercase tracking-wider">Occupants</h4>
                </div>
                <p className="text-sm font-medium text-[#1A2E2A]">{request.numberOfOccupants} persons</p>
              </div>
            </div>
            ...
            <div className="bg-[#F5F9F8] rounded-2xl p-4">
              ...
              <div className="space-y-2">
                <div className="flex items-center justify-between">
                  <span className="text-sm text-[#5A7D78]">Employment Type</span>
                  <span className="text-sm font-medium text-[#1A2E2A]">{request.employmentType}</span>
                </div>
                <div className="flex items-center justify-between">
                  <span className="text-sm text-[#5A7D78]">Company / Organization</span>
                  <span className="text-sm font-medium text-[#1A2E2A]">{request.companyName}</span>
                </div>
                <div className="flex items-center justify-between">
                  <span className="text-sm text-[#5A7D78]">Monthly Income</span>
                  <span className="text-sm font-medium text-[#1A2E2A]">₹{request.monthlyIncome.toLocaleString()}</span>
                </div>
              </div>
            </div>
...
/* ============================================================
   ADD / EDIT RENTAL REQUEST MODAL
============================================================ */

const AddEditRentalRequestModal = ({ request, show, mode, onClose, onSave }) => {
  const emptyForm = {
    tenantName: '',
    tenantEmail: '',
    tenantPhone: '',
  ...
  return (
    <div className="fixed inset-0 z-[70] flex items-center justify-center p-4 bg-[#1A2E2A]/50 backdrop-blur-sm animate-fade-in">
      <div className="bg-white rounded-3xl max-w-2xl w-full max-h-[90vh] overflow-hidden shadow-2xl animate-slide-up border border-[#E8F0EE] flex flex-col">
        ...
        {/* Content */}
        <div className="flex-1 overflow-y-auto p-6 bg-white">
          <form id="rental-request-form" onSubmit={handleSubmit} className="space-y-4">
            <div className="grid grid-cols-2 gap-4">
              {/* Tenant Information */}
...
const RentalRequestDetails = () => {
  ...
  const generateMockRequests = useCallback(() => {
    ...
    const propertyTypes = ['Individual', 'Apartment', 'Commercial', 'Land & Plots', 'Hostel'];
    const furnishingOptions = ['Fully Furnished', 'Semi Furnished', 'Unfurnished'];
    const statuses = ['pending', 'approved', 'rejected', 'under_review', 'completed'];
    const rentalDurations = ['6 months', '12 months', '18 months', '24 months', '36 months'];
  ...
  const filterRequests = useCallback(() => {
    ...
    if (selectedEmploymentType !== 'all') {
      filtered = filtered.filter(req => req.employmentType === selectedEmploymentType);
    }
    ...
    setCurrentPage(1);
  }, [requests, searchQuery, selectedStatus, selectedPropertyType, selectedFurnishing, selectedEmploymentType, sortField, sortDirection]);
    ...
    setRequests(prev => {
      ...
      if (formMode === 'add') {
        const newRequest = {
          ...
          id: `rental_req_${Date.now()}`,
          createdAt: new Date().toISOString()
        };
        updated = [newRequest, ...prev];
      } else {
        updated = prev.map(r => r.id === formRequest.id ? { ...r, ...data, bedrooms: parseInt(data.bedrooms) || 1 } : r);
      }
  ...
  const handleDeleteRequest = useCallback((reqId) => {
    ...
    if (!window.confirm(`Are you sure you want to delete ${req.tenantName}'s rental request?`)) return;
  ...
  const handleStatClick = useCallback((filter) => {
    ...
    if (nextFilter === 'pending' || nextFilter === 'approved' || nextFilter === 'rejected' || 
        nextFilter === 'under_review' || nextFilter === 'completed') {
  ...
  const handleBulkAction = useCallback((action) => {
    ...
    setTimeout(() => {
      ...
      else if (action === 'delete') showToast(`${count} request(s) deleted`, 'error');
    }, 800);
        ...
        <div className="flex flex-col lg:flex-row items-start lg:items-center justify-between gap-4">
          <div>
            ...
            <p className="text-sm text-[#5A7D78] flex items-center gap-2 flex-wrap">
              <span>Manage tenant rental requests</span>
              ...
              return (
                ...
                >
                  ...
                  <div className="space-y-1">
                    ...
                    <div className="flex items-center gap-2 text-[11px] text-[#5A7D78]">
                      <MdOutlineSecurity className="text-[#00695C] flex-shrink-0" />
                      <span>₹{req.securityDeposit.toLocaleString()}</span>
                    </div>
                    <div className="flex items-center gap-2 text-[11px] text-[#5A7D78]">
                    ...
                    <div className="flex items-center gap-2 text-[11px] text-[#5A7D78]">
                      ...
                      <span>{req.numberOfOccupants} occupants</span>
                    </div>
                    <div className="flex items-center gap-2 text-[11px] text-[#5A7D78]">
                      <FaBriefcaseSolid className="text-[#00695C] flex-shrink-0" />
                      <span className="truncate">{req.employmentType}</span>
                    </div>
                  </div>

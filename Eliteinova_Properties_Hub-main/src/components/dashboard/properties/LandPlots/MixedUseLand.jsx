// src/components/dashboard/properties/LandPlots/MixedUseLand.jsx
...
import {
  ...
  FiMap, FiActivity, FiLayers
} from 'react-icons/fi';
import { FaCity } from 'react-icons/fa';

// ============================================================
// FIXED MAIN PROPERTY TYPE — this page only handles Mixed-Use Land
// ============================================================
const MAIN_TYPE_LABEL = 'Mixed-Use Land';
...
const SUB_TYPES = {
  'ResidentialCommercialPlot': {
    ...
    label: 'Residential + Commercial Plot'
  },
  'CommercialIndustrialLand': {
    ...
    label: 'Commercial + Mixed-Use Land'
  },
  'TownshipDevelopmentLand': {
    ...
    label: 'Township Development Land'
  },
  'MultiPurposeDevelopmentLand': {
    ...
    label: 'Multi-purpose Development Land'
  }
};
...
const ViewPropertyDetailModal = ({ property, show, onClose, onEdit, onDelete }) => {
  ...
  const subConfig = SUB_TYPES[property.propertySubType] || SUB_TYPES['ResidentialCommercialPlot'];
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm animate-fade-in">
      <div className="bg-white rounded-3xl max-w-4xl w-full max-h-[90vh] overflow-hidden shadow-2xl animate-slide-up border border-[#E8F0EE] flex flex-col">
        ...
        <div className="sticky top-0 bg-gradient-to-r from-[#00695C] to-[#26A69A] p-6 rounded-t-3xl z-10 shrink-0">
          ...
          <h2 className="text-2xl font-bold text-white">Edit Mixed-Use Land Plot</h2>
...
const MixedUseLand = () => {
  ...
  const generateMockProperties = useCallback(() => {
    const propertyTitles = [
      'Union Square Residential + Commercial Plot', 'Metro Commercial + Industrial Land', 'Skyline Township Development Land',
      'Galaxy Multi-purpose Development Land', 'Silverline Residential + Commercial Plot', 'Crown Commercial + Industrial Land',
      'Riverside Township Development Land', 'Fortune Multi-purpose Development Land', 'Emerald Residential + Commercial Plot',
      'Sunrise Commercial + Industrial Land', 'Meridian Township Development Land', 'Phoenix Multi-purpose Development Land',
      'Junction Residential + Commercial Plot', 'Prime Commercial + Industrial Land', 'Horizon Township Development Land',
      'Palm Grove Multi-purpose Development Land', 'Regal Residential + Commercial Plot', 'Zenith Commercial + Industrial Land'
    ];
    ...
    for (let i = 1; i <= 60; i++) {
      ...
      propertiesList.push({
        id: `mu_${i}`,
        propertyId: `MU-${String(i).padStart(4, '0')}`,
        propertyTitle: propertyTitles[Math.floor(Math.random() * propertyTitles.length)],
        propertyType: MAIN_TYPE_LABEL,
        propertySubType,
        listingType,
        description: `A well-located ${SUB_TYPES[propertySubType].label.toLowerCase()} under mixed-use land, available for ${listingType.toLowerCase()}.`,
  ...
  const handleExport = useCallback(() => {
    ...
    try {
      ...
      a.href = url;
      a.download = `mixed_use_land_${new Date().toISOString().split('T')[0]}.csv`;
      a.click();
        ...
        <div className="flex flex-col lg:flex-row items-start lg:items-center justify-between gap-4">
          <div>
            ...
            <p className="text-sm text-[#5A7D78] flex items-center gap-2 flex-wrap">
              <span>All Mixed-Use Land sub types across Buy, Rent &amp; Lease</span>

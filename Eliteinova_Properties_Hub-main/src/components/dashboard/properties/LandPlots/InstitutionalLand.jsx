// src/components/dashboard/properties/LandPlots/InstitutionalLand.jsx
...
import { FaCity, FaGraduationCap, FaHospital, FaChalkboardTeacher, FaLandmark } from 'react-icons/fa';

// ============================================================
// FIXED MAIN PROPERTY TYPE — this page only handles Institutional Land
// ============================================================
const MAIN_TYPE_LABEL = 'Institutional Land';
...
const SUB_TYPES = {
  ...
  'ReligiousInstitutionLand': {
    ...
    label: 'Religious Institution Land'
  }
};
...
const ViewPropertyDetailModal = ({ property, show, onClose, onEdit, onDelete }) => {
  ...
  const subConfig = SUB_TYPES[property.propertySubType] || SUB_TYPES['SchoolCollegeLand'];
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm animate-fade-in">
      <div className="bg-white rounded-3xl max-w-4xl w-full max-h-[90vh] overflow-hidden shadow-2xl animate-slide-up border border-[#E8F0EE] flex flex-col">
        ...
        <div className="sticky top-0 bg-gradient-to-r from-[#00695C] to-[#26A69A] p-6 rounded-t-3xl z-10 shrink-0">
          ...
          <h2 className="text-2xl font-bold text-white">Edit Institutional Land Plot</h2>
...
const InstitutionalLand = () => {
  ...
  const generateMockProperties = useCallback(() => {
    const propertyTitles = [
      'Bright Future School / College Land', 'Wellness Hospital / Clinic Land', 'Skill Hub Training Institute Plot',
      'Heritage Religious Institution Land', 'Silverline School / College Land', 'Crown Hospital / Clinic Land',
      'Riverside Training Institute Plot', 'Fortune Religious Institution Land', 'Emerald School / College Land',
      'Sunrise Hospital / Clinic Land', 'Meridian Training Institute Plot', 'Phoenix Religious Institution Land',
      'Junction School / College Land', 'Prime Hospital / Clinic Land', 'Horizon Training Institute Plot',
      'Palm Grove Religious Institution Land', 'Regal School / College Land', 'Zenith Hospital / Clinic Land'
    ];
    ...
    for (let i = 1; i <= 60; i++) {
      ...
      propertiesList.push({
        id: `in_${i}`,
        propertyId: `IN-${String(i).padStart(4, '0')}`,
        propertyTitle: propertyTitles[Math.floor(Math.random() * propertyTitles.length)],
        propertyType: MAIN_TYPE_LABEL,
        propertySubType,
        listingType,
        description: `A well-located ${SUB_TYPES[propertySubType].label.toLowerCase()} under institutional land, available for ${listingType.toLowerCase()}.`,
  ...
  const handleExport = useCallback(() => {
    ...
    try {
      ...
      a.href = url;
      a.download = `institutional_land_${new Date().toISOString().split('T')[0]}.csv`;
      a.click();
        ...
        <div className="flex flex-col lg:flex-row items-start lg:items-center justify-between gap-4">
          <div>
            ...
            <p className="text-sm text-[#5A7D78] flex items-center gap-2 flex-wrap">
              <span>All Institutional Land sub types across Buy, Rent &amp; Lease</span>
...
export default InstitutionalLand;
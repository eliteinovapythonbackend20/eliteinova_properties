          ...
          {property.contactPersonDetails && (property.contactPersonDetails.name || property.contactPersonDetails.mobile) && (
            <div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-1.5 sm:gap-2">
                ...
                {property.contactPersonDetails.gstNumber && (
                  <div className="flex items-center gap-1.5 sm:gap-2 p-1.5 sm:p-2 bg-gray-50 rounded-lg">
                    <div className="p-1 sm:p-1.5 bg-[#00695C]/10 rounded-lg"><Award className="w-3 h-3 sm:w-3.5 sm:h-3.5 text-[#00695C]" /></div>
                    <div><p className="text-[9px] sm:text-[10px] text-gray-500 font-bold uppercase tracking-wider">GST No.</p><p className="text-[10px] sm:text-xs font-bold text-gray-800">{property.contactPersonDetails.gstNumber}</p></div>
                  </div>
                )}
              </div>
            </div>
...
const PropertyManagementProfile = () => {
  ...
  const [showCompanyLogoDeleteConfirm, setShowCompanyLogoDeleteConfirm] = useState(false);
  ...
  // ============ FETCH PROPERTY MANAGEMENT PROFILE DATA ============
  const fetchPMProfileData = async () => {
    try {
      setIsLoading(true);
      setError(null);

      const response = await getMyProfile("property-management");
      ...
      setEditForm({
        // Company Information
        pmCompanyName: profileData.companyName || profileData.pmCompanyName || '',
        pmBusinessRegNumber: profileData.businessRegNumber || profileData.companyRegNumber || '',
        pmReraNumber: profileData.reraRegistrationNumber || profileData.reraNumber || '',
        pmGstNumber: profileData.gstNumber || '',
        pmYearsOfExperience: profileData.yearsOfExperience || profileData.experience || '',
        pmCompanyWebsite: profileData.companyWebsite || profileData.website || '',
        pmCompanyLogo: null,
        pmCompanyDescription: profileData.companyDescription || profileData.companyProfile || '',

        // Authorized Representative
        authFullName: profileData.fullName || profileData.authFullName || '',
      ...
      console.error('❌ Error fetching PM profile data:', error);
  ...
  const handleSave = async () => {
    ...
    try {
      const profileData = {
        companyName: editForm.pmCompanyName,
        businessRegNumber: editForm.pmBusinessRegNumber,
        reraNumber: editForm.pmReraNumber,
        gstNumber: editForm.pmGstNumber,
        yearsOfExperience: editForm.pmYearsOfExperience,
        companyWebsite: editForm.pmCompanyWebsite,
        companyDescription: editForm.pmCompanyDescription,
  ...
  const handleProfilePhotoUpload = async (e) => {
    ...
    if (file) {
      setIsLoading(true);
      try {
        const response = await uploadProfilePhoto('property-management', file);
  ...
  // Company logo lives on the dedicated company_logo_url column (see
  // ProfileService.delete_vendor_logo), not in the generic vendor-document
  // JSONB metadata that removeFile()/deleteDocument() manages - routing its
  // delete through that generic path 404s (no such document was ever
  // recorded there, since upload also uses the dedicated /logo endpoint) and
  // never actually clears the logo.
  const handleCompanyLogoDelete = () => {
    setShowCompanyLogoDeleteConfirm(true);
  };

  const confirmCompanyLogoDelete = async () => {
    setIsLoading(true);
    try {
      await deleteVendorLogo('property-management');
      setDocuments(prev => ({
        ...prev,
        companyLogo: null
      }));
      ...
      setShowCompanyLogoDeleteConfirm(false);
      showSuccessToast('Company logo deleted successfully!');
      await fetchPMProfileData();
    } catch (error) {
  ...
  const confirmProfilePhotoDelete = async () => {
    setIsLoading(true);
    try {
      await deleteProfilePhoto('property-management');
      setDocuments(prev => ({
        ...prev,
        profilePhoto: null
      }));
  ...
  const handleToggleStatus = async (property) => {
    ...
    setIsLoading(true);
    try {
      await updateVendorPropertyStatus('property-management', property.id, newStatus);
      setProperties(prev =>
  ...
  const confirmDeleteProperty = async () => {
    setIsLoading(true);
    try {
      await deleteVendorProperty('property-management', propertyToDelete.id);
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-[#00695C]/5 via-teal-50/50 to-[#26A69A]/5 pt-16 sm:pt-20 pb-8 sm:pb-12 w-full relative overflow-hidden">
      ...
      {showCompanyLogoDeleteConfirm && (
        <DeleteConfirmModal title="Delete Company Logo" message="Are you sure you want to delete your company logo? This action cannot be undone."
          onConfirm={confirmCompanyLogoDelete} onCancel={() => setShowCompanyLogoDeleteConfirm(false)} />
      )}
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3 sm:gap-4 w-full">
                  ...
                  <div className="space-y-1 sm:space-y-1.5 w-full">
                    ...
                    <div className="flex items-center gap-2 sm:gap-3">
                      ...
                      {documents.companyLogo && (
                        <button onClick={handleCompanyLogoDelete} className="px-2 sm:px-3 py-1 sm:py-1.5 bg-red-500 text-white rounded-lg text-[10px] sm:text-xs font-bold hover:bg-red-600 transition-all duration-300">Delete</button>
                      )}
        ...
        <div className="relative bg-[#00695C]/5 backdrop-blur-sm rounded-xl sm:rounded-2xl shadow-xl p-4 sm:p-5 md:p-6 mb-4 sm:mb-6 w-full hover:shadow-2xl transition-all duration-500 border border-[#00695C]/20 overflow-hidden group">
          ...
          <div className="flex flex-col md:flex-row items-center md:items-start gap-4 sm:gap-5 md:gap-6 w-full relative z-10">
            <div className="relative flex-shrink-0">
              ...
              {documents.companyLogo && (
                <button onClick={handleCompanyLogoDelete} className="absolute top-0 right-0 p-1 rounded-full bg-white shadow-lg hover:bg-red-500 text-gray-600 hover:text-white transition-all duration-300 hover:scale-110 hover:rotate-12 z-20" aria-label="Delete company logo" title="Delete Company Logo">
                  <Trash2 className="w-2.5 h-2.5 sm:w-3 sm:h-3" />
                </button>
              )}
